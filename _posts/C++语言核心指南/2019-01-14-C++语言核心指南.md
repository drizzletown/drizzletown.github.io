...
*查看指针：将其分为非物主（缺失情况下）和物主两类。
* Look at pointers: Classify them into non-owners (the default) and owners.
  只要可行，就把物主指针替换为标准库资源处理方式（如上文例子所示）
...
* [C.con: 容器和其他资源处理（Containers and other resource handles）](#SS-containers)
* [E: 错误处理（Error handling）](#S-errors)
...
* (简单) 在 `delete` 一个不是 `owner<T>` 的裸指针时，发出警告。建议使用标准库资源处理方式或者使用 `owner<T>`。
