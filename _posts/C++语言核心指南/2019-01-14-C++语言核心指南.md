...
为更好地实现目标而消耗的时间和空间不会浪费（例如，开发速度、资源安全性、测试的简化）。
...
是的，这是一个很夸张的描绘。但是，我们已经看到在生产代码中到处充斥着这样的错误，甚至更糟糕。
...
`buf`的 `new` 和 `delete` 是冗余的；如果我们真的需要一个局部作用域的字符串，就应该使用一个局部作用域的 `string`。
还有更多的性能缺陷和无缘无故的复杂逻辑。
