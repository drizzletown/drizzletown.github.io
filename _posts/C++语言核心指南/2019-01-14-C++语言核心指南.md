...
# <a name="main"></a>C++核心指南（译者：雷小锋，lxf@cumt.edu.cn）
...
这是一个不断改进的文档，如果这是一个开源代码工程，现在会是0.8版。复制、使用、修改以及创建此工程的衍生作品需要遵循MIT-style许可。

为该项目做出贡献需要同意参与者许可证。有关详细信息, 请参阅随附的 [许可] (许可) 文件。
此项目可以提供给 "友好用户" 使用、复制、修改和派生, 希望能有建设性的输入。

非常欢迎改进的评价和建议。随着我们理解的提升、语言和库集合的提升，我们会有计划的修改和扩展本文档。在评论时，请注意[导言](#S-introduction)，其中概括了我们的目标和一般方法。贡献者列表在[这里](#SS-ack)。

问题：

* 规则集合还没有完全检验其完整性、一致性或强制性。
* 三个问号（???）表示已知的缺失信息。
* 更新参考文献部分，许多 C++11 之前的资料太旧了。
* 有关或多或少的最新的待办事项列表, 请参阅:[To-do: Unclassified proto-rules](#S-unclassified)。

你可以[阅读核心指南的范围和结构的解释](#S-abstract)或者直接跳到如下目标：

* [In: 简介 Introduction](#S-introduction)
* [P: 原理 Philosophy](#S-philosophy)
* [I: 接口 Interfaces](#S-interfaces)
* [F: 函数 Functions](#S-functions)
* [C: 类和类层次结构 Classes and class hierarchies](#S-class)
* [Enum: 枚举 Enumerations](#S-enum)
* [R: 资源管理 Resource management](#S-resource)
* [ES: 表达式和语句 Expressions and statements](#S-expr)
* [Per: 性能 Performance](#S-performance)
* [CP: 并发和并行 Concurrency and parallelism](#S-concurrency)
* [E: 错误处理 Error handling](#S-errors)
* [Con: 常量与不可变性 Constants and immutability](#S-const)
* [T: 模板与泛型编程 Templates and generic programming](#S-templates)
* [CPL: C-style编程 C-style programming](#S-cpl)
* [SF: 源代码文件 Source files](#S-source)
* [SL: 标准库 The Standard Library](#S-stdlib)

支持部分（Supporting sections）:

* [A: 体系结构的想法 Architectural ideas](#S-A)
* [NR: 非规则和误解 Non-Rules and myths](#S-not)
* [RF: 参考文献 References](#S-references)
* [Pro: 侧面 Profiles](#S-profile)
* [GSL: 指南支持库 Guidelines support library](#S-gsl)
* [NL: 命名和布局规则 Naming and layout rules](#S-naming)
* [FAQ: 常见问题 Answers to frequently asked questions](#S-faq)
* [Appendix A: 库 Libraries](#S-libraries)
* [Appendix B: 现代化代码 Modernizing code](#S-modernizing)
* [Appendix C: 讨论 Discussion](#S-discussion)
* [Appendix D: 支持工具 Supporting tools](#S-tools)
* [词汇表 Glossary](#S-glossary)
* [待办事项：未分类的原始规则 To-do: Unclassified proto-rules](#S-unclassified)

你可以针对特定的语言特征详细查看如下规则：

* 赋值 assignment:
[正则类型（regular types）](#Rc-regular) --
[推荐的初始化方式（prefer initialization）](#Rc-initialize) --
[复制（copy）](#Rc-copy-semantic) --
[移动（move）](#Rc-move-semantic) --
[其他操作（other operations）](#Rc-matched) --
[缺省（default）](#Rc-eqdefault)
* 类 `class`:
[数据（data）](#Rc-org) --
[不变性（invariant）](#Rc-struct) --
[成员（members）](#Rc-member) --
[辅助成员（helpers）](#Rc-helper) --
[具体类型（concrete types）](#SS-concrete) --
[构造函数、赋值运算和析构函数（ctors, =, and dtors）](#S-ctor) --
[层次结构（hierarchy）](#SS-hier) --
[运算符及其重载（operators）](#SS-overload)
* 概念`concept`:
[规则（rules）](#SS-concepts) --
[泛型编程（in generic programming）](#Rt-raise) --
[模板参数（template arguments）](#Rt-concepts) --
[语义（semantics）](#Rt-low)
* 构造函数 constructor:
[不变性（invariant）](#Rc-struct) --
[建立不变性（establish invariant）](#Rc-ctor) --
[抛出异常（`throw`）](#Rc-throw) --
[缺省（default）](#Rc-default0) --
[不需要的（not needed）](#Rc-default) --
[显式声明（`explicit`）](#Rc-explicit) --
[委托（delegating）](#Rc-delegating) --
[虚构造函数（`virtual`）](#Rc-ctor-virtual)
* 派生类 derived `class`:
[何时使用（when to use）](#Rh-domain) --
[用作接口（as interface）](#Rh-abstract) --
[析构函数（destructors）](#Rh-dtor) --
[复制（copy）](#Rh-copy) --
[getters and setters](#Rh-get) --
[多重继承（multiple inheritance）](#Rh-mi-interface) --
[重载（overloading）](#Rh-using) --
[切片（slicing）](#Rc-copy-virtual) --
[动态类型转换（`dynamic_cast`）](#Rh-dynamic_cast)
* 析构函数 destructor:
[构造和析构（and constructors）](#Rc-matched) --
[何时需要（when needed?）](#Rc-dtor) --
[不能失败（may not fail）](#Rc-dtor-fail)
* 异常 exception:
[错误（errors）](#S-errors) --
[抛出异常（`throw`）](#Re-throw) --
[仅用于错误（for errors only）](#Re-errors) --
[禁止抛出异常（`noexcept`）](#Re-noexcept) --
[最小化try（minimize `try`）](#Re-catch) --
[不能抛出异常（what if no exceptions?）](#Re-no-throw-codes)
* for循环 `for`:
[范围for和for循环（range-for and for）](#Res-for-range) --
[for和while（for and while）](#Res-for-while) --
[for初始化表达式（for-initializer）](#Res-for-init) --
[空语句体（empty body）](#Res-empty) --
[循环变量（loop variable）](#Res-loop-counter) --
[循环变量的类型（loop variable type ???）](#Res-???)
* 函数 function:
[命名（naming）](#Rf-package) --
[单一操作（single operation）](#Rf-logical) --
[不抛出异常（no throw）](#Rf-noexcept) --
[参数（arguments）](#Rf-smart) --
[参数传递（argument passing）](#Rf-conventional) --
[多个返回值（multiple return values）](#Rf-out-multi) --
[指针（pointers）](#Rf-return-ptr) --
[lambda函数（lambdas）](#Rf-capture-vs-overload)
* 内联函数 `inline`:
[小函数（small functions）](#Rf-inline) --
[头文件中的内联函数（in headers）](#Rs-inline)
* 初始化 initialization:
[总是初始化（always）](#Res-always) --
[推荐列表初始化（prefer `{}`）](#Res-list) --
[lambda函数（lambdas）](#Res-lambda-init) --
[类内初始化器（in-class initializers）](#Rc-in-class-initializer) --
[类成员（class members）](#Rc-initialize) --
[工厂函数（factory functions）](#Rc-factory)
* lambda表达式（lambda expression）:
[何时使用（when to use）](#SS-lambdas)
* 运算符 operator:
[约定（conventional）](#Ro-conventional) --
[避免转换运算符（avoid conversion operators）](#Ro-conversion) --
[与lambda函数（and lambdas）](#Ro-lambda)
* 公有、私有和保护（`public`, `private`, and `protected`）:
[信息隐藏（information hiding）](#Rc-private) --
[一致性（consistency）](#Rh-public) --
[保护（`protected`）](#Rh-protected)
* 静态断言 `static_assert`:
[编译期检查（compile-time checking）](#Rp-compile-time) --
[静态断言与概念（and concepts）](#Rt-check-class)
* 结构`struct`:
[用于组织数据（for organizing data）](#Rc-org) --
[无不变性时使用（use if no invariant）](#Rc-struct) --
[无私有成员（no private members）](#Rc-class)
* 模板 `template`:
[抽象（abstraction）](#Rt-raise) --
[容器（containers）](#Rt-cont) --
[概念（concepts）](#Rt-concepts)
* 无符号 `unsigned`:
[无符号与有符号（and signed）](#Res-mix) --
[二进制位操作（bit manipulation）](#Res-unsigned)
* 虚函数 `virtual`:
[接口（interfaces）](#Ri-abstract) --
[非虚（not `virtual`）](#Rc-concrete) --
[析构函数（destructor）](#Rc-dtor-virtual) --
[永不失败（never fail）](#Rc-dtor-fail)

你可以查看如下用于表示规则而设计的概念：

* 断言 assertion: ???
* 错误 error: ???
* 异常 exception: exception guarantee (???)
* 失败 failure: ???
* 不变性 invariant: ???
* 泄露 leak: ???
* 库 library: ???
...
# <a name="S-abstract"></a>摘要（Abstract）

本文档是一组如何更好使用 c++ 的指南，目的是帮助人们有效地使用现代 c++。。所谓“现代C++语言”指的是有效的使用ISO C++标准（当前是C++17，但几乎所有的建议也适用于 C++14 和 C++11)。换句话说，假设你现在开始编码，你希望你的代码五年后看起来像什么？十年后呢？
...
最初的规则集强调一系列形式的安全和简洁，虽然很好但可能过于严苛，我们会引入更多的例外情况以更好的满足现实世界的需求，同时我们也需要更多的规则。

你可能会发现一些规则与你的期望相反，甚至与你的经验相反。此时无论如何都要建议你改变编码方式，否则我们就太失败了。请尝试验证或者反驳这些规则，我们真诚地希望对我们的一些的规则提供测度或优秀案例支持。

你可能会发现一些规则显而易见甚至微不足道。请注意，核心指南的目标之一是帮助经验匮乏的程序员、或者来自不同背景或语言的程序员快速成长。

核心指南中许多规则的设计都可以由分析工具支持，违反规则的情况会被标记并引用或链接到相关的规则。在编写代码之前，不要想着记住所有的规则。

我们有意识地逐步把核心指南的规则引入到一个代码库中，为此我们计划建立工具，并希望其他人也这么干。

非常欢迎改进的评论和建议。随着我们理解的提升、语言和库集合的提升，我们会有计划的修改和扩展本文档。

# <a name="S-introduction"></a>In: 简介（Introduction）

这是现代 c++ (当前 C++17) 的核心指南集合, 其中考虑到未来可能的增强和 iso 技术规范 (TSs)。
其目的是帮助 c++ 程序员编写更简单、更高效、更易于维护的代码。

简介的概要（Introduction summary）:

* [In.target: 目标读者（Target readership）](#SS-readers)
* [In.aims: 目标（Aims）](#SS-aims)
* [In.not: 不是目标（Non-aims）](#SS-non)
* [In.force: 强制性（Enforcement）](#SS-force)
* [In.struct: 文档结构（The structure of this document）](#SS-struct)
* [In.sec: 主要部分（Major sections）](#SS-sec)

## <a name="SS-readers"></a>In.target: 目标读者（Target readership）

所有的C++程序员，包括[需要考虑C语言的程序员](#S-cpl).

## <a name="SS-aims"></a>In.aims: 目标（Aims）

本文的目标是帮助开发人员采用现代C++(当前 C++17), 并在代码库中实现更统一的风格。

我们不会妄言每一条规则都可以有效地应用于所有的代码库，升级旧系统是很困难的。但是, 我们确实认为, 使用规则的程序更不容易出错, 更易于维护。很多时候，规则还会导致更快、更轻松的初始开发。

就我们所知，依据这些规则编写的代码，其性能与旧的传统技术一样好或者更为优越。这些代码遵循零开销原则（zero-overhead principle），意思是无需为不使用的东西支付开销，或者只要抽象机制使用合理则我们至少应该与使用底层语言手工编码获得同样的性能。编写新代码时把这些规则当成是理想，工作在旧代码上时把这些规则当成可以利用的机会，努力逼近这些理想。
记住（Remember）:

### <a name="R0"></a>In.0: 不要恐慌（Don't panic!）

花点时间了解指导规则对你的程序的影响。
这些指导规则是根据“超集的子集化（subset of superset）”原则设计的([Stroustrup05](#Stroustrup05))。
核心指南不是简单地定义裁剪出一个要使用的C++语言子集来实现可靠性、安全性、性能或其他；
相反，我们强烈建议使用一些简单的“扩展组件”([库组件（library components）](#S-gsl))，在“扩展组件”中使用C++那些最容易出错的语言特性，
然后我们就可以在规则集合中禁止使用这些易于出错的语言特性。

核心指南的规则强调静态类型安全（static type safety）和资源安全（resource safety）。因此，指南强调尽可能的进行范围检查、避免解引用空指针（`nullptr`）、避免孤悬指针（dangling pointers）并且通过RAII系统性地使用异常。为了达到这个目标并且为了最大限度地减少令人费解的模糊代码（往往是出错之源），核心指南的规则还强调简单性（simplicity）和在规格化良好的接口(well-specified interfaces)背后隐藏必要的复杂性。

> RAII（Resource Acquisition Is Initialization），资源获取即初始化，是常用的管理资源、避免内存泄露的方法。

核心指南的许多规则带有处方性质（prescriptive）。我们不愿意只是简单的陈述“不要那样做”而不给出可行方案。
这样做的结果是一些规则仅仅是启发性的，不够精确且无法机械地验证检查。

其他规则主要阐明了一般原则。在这些更一般的规则之下更详细和具体的规则提供了部分检查。

这些指导规则阐述了C++语言的核心及其使用方法。我们预计大多数大型组织、特定的应用领域, 甚至大型项目都需要进一步的规则, 可能需要进一步的限制, 以及进一步的库支持。例如，硬件实时编程的程序员通常不能自由地使用空闲存储（动态内存），在选择库时将受到限制。我们鼓励制定更具体的规则, 作为这些核心指南的增编。构建理想的小型基础库并使用它,而不是把你的编程语言层次降低到美妙的汇编代码。

核心指南的规则设计允许[渐进采纳（gradual adoption）](#S-modernizing).

一些规则的目标是增加各种形式的安全性，另一些规则的目标是减少故障的可能性，有些规则可能兼顾两个目标。

旨在防止故障的指导规则经常会禁止一些合法的C++特性。然而，如果表达同一个想法有两种途径时，前者被证明是错误之源而后者则没有，此时我们会努力引导程序员采用后者。

## <a name="SS-non"></a>In.not: 不是目标（Non-aims）

核心指南的规则不是最小化的，也不是正交的。
尤其是一般化的规则虽然简单，但是不能强制执行（unenforceable）。此外，一般化的规则经常很难理解其意义。
更特殊的规则往往更容易理解和执行, 但如果没有一般规则, 这些特殊规则就只是一长串特殊情况。
我们提供帮助新手的规则以及支持专家使用的规则。有些规则可以完全强制执行，另一些则是启发性的。

如同读书，核心指南的规则不一定要顺序约定。你可以使用链接浏览规则，然而，链接的主要用途目标是支持工具。也就是说，支持工具会查找违反规则的情况并返回违例规则的链接，通过链接可以得到违例原因，违例潜在后果的例子和建议的修复方法。

核心指南没有打算替代C++语言教程文章。如果你需要一个针对某种经验级别的程序员教程，看看[参考文献（the references）](#S-references).

核心指南不是如何将旧 c++ 代码转换为现代C++代码的指南。核心指南的目标是以一种具体的方式阐述编写新代码的想法。当然，一些可能的代码现代化/代码更新/代码升级(modernizing/rejuvenating/upgrading)的方法, 请参见[现代化部分（the modernization section）](#S-modernizing)。重要的是, 规则指南支持渐进采纳：一次完成大型代码库的完全转换时不可行的。

核心指南的规则并不要求在每一个语言技术细节中都保证完整或准确的。
有关语言定义问题的最终陈述, 包括一般规则的每个例外和每个功能特性, 请参阅 ISO C++标准。

核心指南的规则并非强迫程序员在贫化版的C++子集下编写代码。核心指南意图强调一个像Java语言那样的C++子集，当然并不想定义这样一个子集。核心指南重视语言的表现力和毫不妥协的性能。

核心指南的这些规则不是价值中立的。与大多数现有的C++代码相比, 核心指南旨在使代码更简单、更正确/安全, 而不丢失性能。核心指南试图完美地限制一些合法有效的C++代码，因为这些代码与错误、额外复杂性和性能低下紧密相关。

核心指南并没有精确到程序员或机器不假思索就可以盲目遵从的地步。强制执行部分试图做到这一点，但是比起精确地或错误地规定一些东西，我们更愿意让规则或定义模糊一点且对解释开放。有时候，精确只能来自于时间和经验，设计不仅仅是数学的一种形式。

核心指南的规则并不完美。

* 一条规则可能造成危害，由于它禁止了在特定情况下有用的事情。
* 一条规则可能造成危害，由于它未能禁止在特定情况下导致严重错误的事情。
* 一条规则可能造成很大危害，由于它的模糊、歧义和不可强制执行(unenforceable)，或者由于它使问题的所有解法都变得可行。

不可能完全做到“不做有害的事情（do no harm）”的标准。当然，核心指南的目标没有那么雄心勃勃，“使大多数程序员在大多数情况下做有益的事情（Do the most good for most programmers）”是核心指南的目标准则。如果你不能接受一条规则，可以反对它、忽略它，但是不要把它淡化到毫无意义；还可以建议进行改进。




## <a name="SS-force"></a>In.force: 强制性（Enforcement）

对于大型代码库，没有强制的规则是不可管理的。但是，所有规则都强制执行，就只能用于小型的弱规则集或者特定的用户社区。

* 但是我们有很多的规则，我们期望规则适用于所有的人。
* 但是不同的人群有不同的需求。
* 但是人们不喜欢阅读太多的规则。
* 但是人们记不住太多的规则。

所以，我们需要取子集（subsetting）来满足各种需求。

* 但是，随意地取子集会导致混乱。

我们希望核心指南帮助更多的人们，使编码更加统一，并且强烈鼓励人们编写现代化的代码。
我们希望鼓励最佳实践，而非把一切都留给个人选择和管理压力。
理想的做法是使用所有规则，获得最大的收益。

这就造成了一些两难困境。
我们试图使用支持工具来解决这些问题。
每条规则都有一个**强制性（Enforcement）**部分，列出了强制执行的想法。
强制执行，可以通过代码评审（code review）、静态分析（static analysis）、编译器、或者运行期检查等支持工具来实现。
只要可能，我们更愿意机械检查（鉴于人类缓慢、不精确和容易厌倦的本性）和静态检查。
只有在没有替代方法存在的情况下，才建议运行期检查，我们才不想引入"分布式脂肪（distributed fat）"。

在适当的情况下, 我们会在**强制性（Enforcement）**部分用相关规则的分组名（称为"侧面profiles"）对一条规则进行标记。
一条规则可以参与几个侧面，或者没有侧面。一开始，我们只有少量侧面用于常见的需求（期望，理想）：

* **类型（type）**: 没有类型冲突（通过cast类型转换、联合或可变参数将一个`T`重新解释为一个`U`）
* **边界（bounds）**: 没有边界冲突（访问超越数组范围）
* **生命期（lifetime）**: 没有泄露（没有`delete`或者多个`delete`）且没有访问无效对象（解引用空指针`nullptr`，使用孤悬引用）。

这些侧面是供支持工具使用的，也可以帮着人类阅读。
我们在**强制性（Enforcement）**部分的说明并不局限于已知如何强制执行的事物，一些说明仅仅是愿望，或许能够激发起一些工具开发者的兴趣。

实现这些规则的支持工具，应该遵从如下语法来明确地禁止一条规则：

    [[gsl::suppress(tag)]]

这里，"tag"是强制执行规则所在位置的项目的锚链名称（例如，对于[C.134](#Rh-public)，tag就是"Rh-public"），规则组的侧面名称（"type", "bounds", or "lifetime")，或者侧面中的一条特定规则([type.4](#Pro-type-cstylecast), 或者 [bounds.2](#Pro-bounds-arrayindex))。

## <a name="SS-struct"></a>In.struct: 文档结构（The structure of this document）

每条规则（原则，建议）具有如下几个部分:

* 规则本身 -- 例如，**no naked `new`**
* 规则引用编号 -- 例如，**C.7** (与classes相关的第7条规则)。
  由于主要部分并非内在有序，所以我们使用字母作为规则引用编号的首部。当我们添加或者移除规则时会保留空白编号，从而最大限度减少中断。
* **原因（Reason）**或理论基础（rationales） -- 程序员很难遵循他们不理解的规则
* **例子（Example）** -- 规则仅仅停留在抽象层面上很难理解，可以给出正面或反面的例子
* **替代方案（Alternative）** -- 用于“不这样做”的规则
* **例外（Exception）** -- 我们更喜欢简单一般的规则。然而, 许多规则适用广泛但并不普适,此必须列出例外情况。
* **强制性（Enforcement）** -- 关于规则可能如何进行机械检查的想法。
* **另请参见（See also）** -- 指向相关规则或者进一步讨论的引用（可能在本文档中或者其他地方）
* **注释（Note, comments）** -- 需要说明但不适合放到其他部分的事情
* **讨论（Discussion）** -- 指向更广泛的理论基础或者放置在规则的主要列表之外的示例

有些规则很难实现机械检查，但是这些规则都符合最低标准：一个专家程序员不用太费劲就可以发现许多违例冲突。
我们希望机械检查工具会随着时间的推移而改进，逐渐接近一个专家程序员所发现的东西。
另外, 我们也假设这些规则会随着时间的推移而不断精炼, 变得更精确，可以机械检查。

规则就要简单，不需要仔细措辞以考虑每种替代方法和特殊情况。关于这方面的信息放在**替代方案（Alternative）**段落和[讨论（Discussion）](#S-discussion)部分。
如果你不理解或者不同意一条规则，不妨访问规则的**讨论（Discussion）**。
如果你觉得讨论缺失或者不完整，进入一个[问题(Issue)](https://github.com/isocpp/CppCoreGuidelines/issues)解释你的考虑和possibly a corresponding PR.

核心指南不是语言手册，目标是有帮助，而非完整、技术细节精确或者是对现有代码的指南。
推荐的信息来源可以在[参考文献](#S-references)中找到。

## <a name="SS-sec"></a>In.sec: 主要部分（Major sections）
...
每个部分（例如"P"代表的"原理Philosophy"）、每个子部分（例如，"C.hier"代表的"类层次结构（OOP）"）都有一个缩写，用于搜索和引用。主要部分的缩写也用于规则编号（例如，"C.11"表示"使具体类型正则化（Make concrete types regular）"）。

# <a name="S-philosophy"></a>P: 原理（Philosophy）
...
* [P.6: 编译期不能检查的应该在运行期可检查（What cannot be checked at compile time should be checkable at run time）](#Rp-run-time)
* [P.7: 尽早捕捉运行期错误（Catch run-time errors early）](#Rp-early)
* [P.8: 不要泄露任何资源（Don't leak any resources）](#Rp-leak)
* [P.9: 不要浪费时间和空间（Don't waste time or space）](#Rp-waste)
* [P.10: 不可变数据优先于可变数据（Prefer immutable data to mutable data）](#Rp-mutable)
* [P.11: 封装杂乱的结构，不要散布在代码中（Encapsulate messy constructs, rather than spreading through the code）](#Rp-library)
* [P.12: 恰当的使用支持工具（Use supporting tools as appropriate）](#Rp-tools)
* [P.13: 恰当的使用支持库（Use support libraries as appropriate）](#Rp-lib)

原理性规则一般不能机械检验，但是，反映这些原理主题的单个规则是可检验的。
没有原理性规则的基础，则更加具体、特定、可检验的规则就没有了基础理论支持。


### <a name="Rp-direct"></a>P.1: 直接用代码表达想法（Express ideas directly in code）

##### 原因（Reason）

编译器不会阅读注释或者设计文档，许多程序员也不会阅读注释或者设计文档。
代码所表达的就是语义，原则上应该能够通过编译器或者其他工具进行检查。
...
第一个`month`的声明非常明确，返回一个`Month`并且不会修改`Date`对象的状态。
第二个`month`的声明版本则会让读者胡乱猜测，更有可能导致未捕获的bug。

##### 不好的例子（Example; bad）

如下的循环，是`std::find`的约束版本：
...
通过一个良好设计的库来表达意图（要做什么，而非事情如何做），远优于直接使用语言特性。

* C++程序员应该了解标准库的基础知识，并恰当的使用它。
* 任何程序员都应该了解所参与工程项目的基础库的基础知识，并恰当的使用它。
* 任何使用本核心指南的程序员，都应该了解[指南支持库（guidelines support library）](#S-gsl)，并恰当的使用它。

##### 示例（Example）

    change_speed(double s);   // bad: what does s signify?（s表示的含义不清晰）
    // ...
    change_speed(2.3);

更好的解决方法是明确double参数的意义（是新的速度还是旧速度的改变量）以及使用的单位：

    change_speed(Speed s);    // better: the meaning of s is specified
    // ...
    change_speed(2.3);        // error: no unit（没有单位）
    change_speed(23m / 10s);  // meters per second（米每秒）

当然可以接受一个简单的无单位`double`作为改变量delta，但是这容易导致编程出错。
如果绝对速度和改变量两者我们都想要，最好定义一个`Delta`类型。

##### 强制性（Enforcement）

一般来说很难。

* 始终使用`const`（检查成员函数是否修改对象状态；检查函数是否修改指针或引用传递过来的参数）
* 标记类型转换的使用(把中性词转换为类型系统（casts neuter the type system）)
* 检测与标准库相似的代码（很难）

### <a name="Rp-Cplusplus"></a>P.2: 使用ISO标准C++编写程序（Write in ISO Standard C++）

##### 原因（Reason）

核心指南是一套编写ISO标准C++程序的指南。

##### 注解（Note）

在某些环境下需要扩展组件，例如，访问系统资源。
在这种情况下，对扩展组件的使用进行本地化，通过非核心编码指南控制其使用。
如果可能，构建封装扩展组件的接口，以便在不支持这些扩展组件的系统上关闭或者编译它们。

扩展组件通常没有严格定义的语义。甚至一些常用的扩展组件由多种编译器实现，没有严格的标准定义，直接结果就是扩展组件可能会有一些不同的行为和边界情况下的行为。大量地使用这样的扩展组件，预期的可移植性会受到影响。

##### 注解（Note）

使用有效的ISO C++并不能保证可移植性(更不要说正确性了）。
避免依赖于未定义行为（例如，[未定义的求值顺序（undefined order of evaluation）](#Res-order))，注意依赖于实现定义的结构（例如，`sizeof(int)`）。

##### 注解（Note）

在某些环境下需要限制使用标准C++语言或者库特性，例如，飞机控制软件标准要求避免动态内存分配。
在这种情况下，可以针对特定环境进行定制，通过这些编码指南的扩展来控制语言或库特性的使用与否。

##### 强制性（Enforcement）

使用最新的C++编译器（当前C++17、C++14或C++11）和一组不接受扩展的选项。

### <a name="Rp-what"></a>P.3: 表示意图（Express intent）

##### 原因（Reason）

除非描述某些代码的意图（例如，通过名字或注释），否则不可能判断代码是否完成了它应该做的事情。
...
这里，只是在`v`的元素上循环，循环的意图是什么却没有表达。index的实现细节暴露出来（可能是误用），变量`i`存活到循环作用域之外，这可能是故意的，也可能不是。总之，读者无法仅仅从这部分代码中获知这些信息。

更好的表达方式是（Better）:

    for (const auto& x : v) { /* do something with the value of x */ }

现在，没有明确提到迭代循环机制，并且循环操作发生在`const`元素的引用上，因此元素的意外修改不可能发生。
如果想修改元素，可以这样：

    for (auto& x : v) { /* modify x */ }

关于for语句的更多细节，参见[ES.71](#Res-for-range)。
有时候还可以更好，使用一个特定算法。这个示例使用来自Ranges TS中的`for_each`，可以直接表达我们的意图：

    for_each(v, [](int x) { /* do something with the value of x */ });
    for_each(par, v, [](int x) { /* do something with the value of x */ });

最后这个变体清晰地表明，我们并不关心`v`的元素处理顺序。'par'是'std::execution::par'，表示并行执行策略；[]表示这是C++的lambda函数。

程序员应该熟悉：

* [指南支持库（The guidelines support library）](#S-gsl)
* [ISO标准库（The ISO C++ Standard Library）](#S-stdlib)
* 工程项目所使用的基础库

##### 注解（Note）

该规则的替代表述：描述应该做什么，而不仅仅是怎么做。

##### 注解（Note）

某些语言结构比其他语言结构更易于表达意图。

##### 示例（Example）

如果两个`int`被用于表示二维点的坐标，假设这样做：

    draw_line(int, int, int, int);  // obscure（模糊）
    draw_line(Point, Point);        // clearer（更清晰）
...
这里为你的聪明才智和半自动的程序转换提供了广阔的发挥空间。

### <a name="Rp-typesafe"></a>P.4: 理想情况下程序应是静态类型安全的（Ideally, a program should be statically type safe）

##### 原因（Reason）

理想情况下，一个程序应该是完全静态（编译期）类型安全的。很不幸这不可能，问题涉及的场合包括：

* 联合（unions）
* 类型转换（casts）
* 数组退化（array decay）
* 范围错误（range errors）
* 收缩转换（narrowing conversions）

##### 注解（Note）

这些场合是严重问题发生的源头（例如，崩溃和安全冲突），我们试图提供替代技术。

##### 强制性（Enforcement）

我们可以根据需要或单个程序的可行性分别禁止、抑制、或者检测各个问题类别。
必须给出替代方案的建议。
例如：

* unions -- 使用 `variant` (in C++17)
* casts -- 尽可能少用；使用模板是有帮助的
* 数组退化 -- 使用 `span` (来自于指南支持库GSL)
* 范围错误 -- 使用 `span`
* 收缩转换 -- 尽可能少用；如果必须就使用`narrow` 或 `narrow_cast` (来自指南支持库GSL)

### <a name="Rp-compile-time"></a>P.5: 编译期检查优先于运行期检查（Prefer compile-time checking to run-time checking）

##### 原因（Reason）

代码清晰度和性能。
无需为编译期捕获的错误编写错误处理器（error handler）。

##### 示例（Example）

    // Int是整型的一个别名（Int is an alias used for integers）
...
这个例子得不到想要的结果（因为溢出是未定义的），应该使用简单的`static_assert`来替代：

    // Int is an alias used for integers
    static_assert(sizeof(Int) >= 4);    // do: compile-time check

或者可能更好，只使用语言的类型系统将`Int` 替换为 `int32_t`。
...
更好的方法：
    ...
    int a[100];
    read(a);        // better: 让编译器计算元素个数

**替代表述（Alternative formulation）**: 编译器能做好的事情不要拖延到运行期。
...
### <a name="Rp-run-time"></a>P.6: 编译期无法进行的检查应该在运行期实现检查（What cannot be checked at compile time should be checkable at run time）

##### 原因（Reason）

在程序中保留难以检查的错误，无异于自己请求崩溃和糟糕的结果。

##### 注解（Note）

理想情况下，我们在编译期或者运行期捕获所有错误（不是程序员的逻辑错误）。在编译期捕获所有错误是不可能的，而且剩余错误通常也无法在运行时全部捕获。 但是，如果有足够的资源（分析程序，运行期检查，机器资源，时间），我们应该尽量编写原则上可以检查的程序。
...
这里，元素个数这个关键信息被彻底隐藏，使得静态分析变得几乎不可行；如果`f()`是ABI的一部分使得我们无法解释那个指针时，动态检查也非常困难。当然，我们可以把有用的信息嵌入空闲存储空间，但这就需要对系统（可能是编译器）进行全局性的改变。我们这里的设计使错误检测非常困难。

> ABI（应用二进制接口，application binary interface），是两个二进制程序模块之间的接口。ABI在机器码层面定义如何访问数据结构和函数，比较底层且依赖于硬件；与之对应，API在源码层面定义如何访问数据结构和函数，比较高层且人类可读。
...
我当然可以传递指针以及元素个数：
    ...
    void g2(int n)
    {
        f2(new int[n], m);  // bad: 可能传递错误的元素个数给f()
    }

传递元素个数参数是更好且很常用的方法，远好于只传递指针然后依赖于某些（未说明的）约定来了解发现元素个数。但是，就像上面代码所示的那样，一个简单的拼写错误（不小心把 'n' 敲成了 'm'）就会导致严重错误，`f2()`函数两个参数之间的关系是一种心照不宣的约定，而非明确的宣示。

此外，这里还隐含地假设`f2()`函数会调用`delete`释放其参数（或许调用者会犯另一个错误，也调用`delete`释放参数）。

##### 不好的例子（Example, bad）

标准库的资源管理指针在指向一个对象时无法传递该对象的大小：
    ...
    void g3(int n)
    {
        f3(make_unique<int[]>(n), m);    // bad: 分别传递物主对象（ownership）及其大小
    }

##### 示例（Example）

最好是把指针和元素个数作为一个完整对象传递：
    ...
    {
        vector<int> v(n);
        f4(v);                     // 传递一个引用，pass a reference, retain ownership
        f4(span<int>{v});          // 传递一个视图，pass a view, retain ownership
    }

这种设计把元素个数作为对象的组成部分，因此不可能出错，动态的运行期检查总是可行的，即使动态检查并不一定值当。

##### 示例（Example）

怎么样传递物主对象及其有效使用的所有信息？

    vector<int> f5(int n)    // OK: move（移动拷贝）
    {
        vector<int> v(n);
        // ... initialize v ...
        return v;
    ...
    unique_ptr<int[]> f6(int n)    // bad: loses n（丢失n）
    {
    ...
    owner<int*> f7(int n)    // bad: loses n and we might forget to delete（丢失n而且可能忘记delete）
    {
 ...
 * 当接口确实知道需要什么的时候，怎么样通过接口来传递多态基类以避免可能的检查？或许strings是自由风格的可选项。

##### 强制性（Enforcement）

* 标记(pointer, count)-style的接口（这将标记出许多由于兼容性原因而无法修复的示例）
* Flag (pointer, count)-style interfaces (this will flag a lot of examples that can't be fixed for compatibility reasons)
* ???

### <a name="Rp-early"></a>P.7: 及早捕获运行期错误（Catch run-time errors early）
...
这里，我们在`use1`中制造了一个小错误，会导致损坏的数据或者程序崩溃。
(pointer, count)-style的接口，使`increment1()`缺乏现实的途径抵御越界错误。
即使我们能检查越界访问的下标，那么在访问`p[10]`之前也不会发现错误。
我们要及早检查并改进代码：
...
现在，`m <= n`会在调用点被及早而非延迟检查。
如果拼写错误使我们打算使用`n`作为边界，则代码可以进一步简化（消除任何错误的可能性）：
    ...
    void use3(int m)
    {
        ...
        int a[n] = {};
        // ...
        increment2(a);   // a的元素个数不必重复
        // ...
    }

##### 不好的例子（Example, bad）

不要重复检查同一个值；不要把结构化数据作为字符串传递：
...
date被验证了两次（由`Date`构造函数），并且被作为字符串（无结构数据）传递。

##### 示例（Example）

过度检查可能代价高昂。有时候及早检查可能非常愚蠢，因为你甚至根本不需要这个值，或者仅仅需要该值的一部分（比整体检查更容易一些）。
类似地，不要因增加验证检查改变你的接口的渐进复杂性（例如，不要在平均复杂度`O(1)`的接口中增加`O(n)`的检查）。
    ...
    public:
        Jet(float x, float y, float z, float e)
            :x(x), y(y), z(z), e(e)
        {
            // Should I check here that the values are physically meaningful?（是否需要在这里检查一下值在物理上是有意义的）
        }

        float m() const
        {
            // Should I handle the degenerate case here?（是否需要处理退化情况）
...
喷气飞机的物理法则(`e * e < x * x + y * y + z * z`)不是永恒的，由于可能存在测量误差。

???

##### 强制性（Enforcement）

* 查看指针和数组：及早进行范围检查且不要重复
* 查看类型转换：消除或者标记出收缩转换（narrowing conversions）
* 查看来自输入的未检查值
* 查看结构化数据（具有不变性的类对象--objects of classes with invariants）被转换为字符串的情况
* ???

### <a name="Rp-leak"></a>P.8: 别泄露任何资源（Don't leak any resources）

##### 原因（Reason）

随着时间的推移，即使缓慢增长也会使资源耗尽，丧失可用性。
这一点虽然只对于长期运行的程序特别重要，但却是负责任的编程行为的必备要素。
...
**另请参见（See also）** [资源管理部分（The resource management section）](#S-resource)

> RAII（Resource Acquisition Is Initialization），资源获取即初始化，通过局部自动对象的构造和析构实现资源管理、避免内存泄露。


##### 注解（Note）

通俗地讲，所谓泄露就是“任何没有清理干净的东西。”
更重要的一类泄露是“任何不再能被清理干净的东西。”
例如，在堆内存分配一个对象，然后丢失了指向该内存的最后一个指针。
这条规则不应该被解读为要求：在程序关闭期间长生命期的对象必须归还其分配内存。
例如，依靠系统保证的清理行为（如在进程关闭时进行文件关闭和内存释放）可以简化代码。
但是，依靠隐含清理行为的抽象则更为简单，通常更为安全。

##### 注解（Note）

强制执行[生命期的安全性侧面（the lifetime safety profile）](#SS-lifetime)会消除泄露。
当它与[RAII](#Rr-raii)提供的资源安全性相结合时，就消除了垃圾收集（garbage collection）的必要（因为没有产生垃圾）。
当它与[类型和边界侧面（the type and bounds profiles）](#SS-force)的强制执行相结合时，就获得了支持工具保证下的完全的类型安全和资源安全（type- and resource-safety）。

##### 强制执行（Enforcement）

*查看指针：将其分为非物主（缺省情况下）和物主两类。
* Look at pointers: Classify them into non-owners (the default) and owners.
  只要可行，就把物主指针替换为标准库的资源句柄方式（如上文例子所示）
  可选地，使用[指南支持库the GSL](#S-gsl)中的“owner”标记出物主指针。
* 查找直接调用的 `new` and `delete`
* 查找已知的返回裸指针的资源分配函数（如`fopen`, `malloc` 和 `strdup`）

### <a name="Rp-waste"></a>P.9: 别浪费时间和空间
...
为更好地实现目标而消耗的时间和空间不是浪费（例如，开发速度、资源安全性、测试的简化）。
"追求效率的另一个好处是这个过程会迫使你更深入的理解问题（Another benefit of striving for efficiency is that the process forces you to understand the problem in more depth）。" - Alex Stepanov
...
是的，这是一个很夸张的说明示例。但是，我们已经看到在生产代码中到处充斥着这样的错误，甚至更糟糕。
注意，`X`的内存布局导致至少浪费了6个字节（可能更多）

> 对齐（alignment）：数据结构或类的成员变量，并非按大小一个接一个紧凑地排列在内存空间中，而是按照某种特定的方法排列的，有可能在两个成员变量之间插入一个或多个byte，以保证每个成员变量的起始位置都从某些特定的位置开始。

拷贝操作的不合理定义使move语义失效，导致返回操作效率低下。
(请注意，这里不保证进行返回值优化——Return Value Optimization, RVO)。
`buf`的 `new` 和 `delete` 是冗余的；如果我们真的需要一个局部作用域的字符串，就应该定义一个局部作用域的 `string`。
还有更多的性能缺陷和无缘无故的复杂逻辑。
...
是的，这是一个来自实际生产代码的例子，请读者指出时间和空间的浪费情况。

##### 注解（Note）

关于浪费的单个例子基本上意义不大，即使有通常也很容易被专家消除。
但是，广泛分布在代码库中的浪费的情况就变得意义重大，专家并不能总是找到问题所在。
本规则（以及更具体的支持规则）的目标，是在发生之前就提早消除掉与C++使用相关的大多数浪费情况。
之后，我们可以查看与算法和需求相关的浪费情况，当然这已经超出了核心指南的范畴。

##### 强制性（Enforcement）

许多更具体的规则，目标是全局的简单性和消除无缘无故浪费情况。

### <a name="Rp-mutable"></a>P.10: 不可变数据优先于可变数据（Prefer immutable data to mutable data）

##### 原因（Reason）

在常量上建立推理比在变量上推理更容易。事物不可变意味着不可能发生出乎意料的变化，有时候基于不变性可以实现更好的优化，在常量上不会发生数据竞争的情况。

参见 [Con: Constants and immutability（常量与不变性）](#S-const)

### <a name="Rp-library"></a>P.11: 封装杂乱的结构，不要散布在代码中（Encapsulate messy constructs, rather than spreading through the code）
...
这是抽象层次低的、冗长的、易于出错的代码示例。
例如，我们还好像忘记了测试内存耗尽的情况。
更好的方法是使用 `vector`:
...
标准库和指南支持库GSL就是封装这一理念的产物。
例如，与其被数组array、联合union、类型转换cast、复杂的生命期问题、`gsl::owner`等等搞得昏头转向，
不如使用比我们更专业的领域专家设计和实现的库，如`vector`、`span`、`lock_guard` 和 `future`这些实现了关键抽象的库。
类似的，我们能够也应该设计并实现更为具体的库，而不是让用户（通常是我们自己）去直面重复编写高质量低层代码的挑战。
本规则是[超集的子集化原则（subset of superset principle）](#R0)的变体，是核心指南的基础。

##### 强制性（Enforcement）

* 在抽象实现之外查找指针操作和强制类型转换这些“混乱代码”。


### <a name="Rp-tools"></a>P.12: 恰当的使用支持工具（Use supporting tools as appropriate）

##### 原因（Reason）

有很多事情机器很擅长，计算机不会疲倦，也不会因重复性的工作而厌倦。
我们人类应该去做更有意义的事情，而非重复性的常规工作

##### 示例（Example）

运行一个静态分析器，检查你的代码是否遵循你想要的指导规则。
...
* [并发工具（Concurrency tools）](#Rconc-tools)
* [测试工具（Testing tools）](???)

还有许多其他的工具，如源代码库、构建工具等，这些已经超出了核心指南的范畴。

##### 注解（Note）

注意不要依赖于过度精心设计的或者过度特化的工具链。
否则，这些会导致你的可移植代码变得不可移植。

### <a name="Rp-lib"></a>P.13: 恰当的使用支持库（Use support libraries as appropriate）

##### 原因（Reason）

使用设计良好、文档齐全、支持良好的库，可以节省时间和精力；
库的质量和文档可能比你做的更好，因为我们的主要时间和精力必须放在实现上面。
使用一个库的成本（时间、精力、金钱等等）可以由许多用户共同分担。
一个广泛应用的库可能更新更及时，比单个应用程序更易于移植到新系统。
学习一个广泛应用的库，可以在其他未来项目上节省时间。
所以，如果在你的应用领域有合适的库存在，学习并使用它。

##### 示例（Example）

    std::sort(begin(v), end(v), std::greater<>());

如果你不是排序算法专家且没有充足的时间，那么这个例子排序代码比起你编写的任何特定应用代码可能都要更正确，运行起来更快。
对于标准库（或其他基础库），如果你不打算使用它就要找到充分的理由，否则就无条件使用它。

##### 注解（Note）

默认使用：

* [ISO标准库（ISO C++ Standard Library）](#S-stdlib)
* [指南支持库（Guidelines Support Library，GSL）](#S-gsl)

##### 注解（Note）

如果在一个重要的领域没有设计良好的、文档齐全的、支持良好的库存在，或许你应该设计并实现这样一个库，然后使用它。


# <a name="S-interfaces"></a>I: 接口（Interfaces）

接口是程序的两个部分之间的契约，准确地陈述服务提供者和服务使用者的期望是接口的本质。
具有良好的接口（易于理解、支持高效使用、不易出错、支持测试等）可能是代码组织最重要的工作方面。

接口规则概要（Interface rule summary）:

* [I.1: 使接口清楚明白（Make interfaces explicit）](#Ri-explicit)
* [I.2: 避免非`const`的全局变量（Avoid non-`const` global variables）](#Ri-global)
* [I.3: 避免单例（Avoid singletons）](#Ri-singleton)
* [I.4: 使接口精确并强类型化（Make interfaces precisely and strongly typed）](#Ri-typed)
* [I.5: 描述前置条件（如果有）（State preconditions (if any)）](#Ri-pre)
* [I.6: 优先使用`Expects()`表示前置条件（Prefer `Expects()` for expressing preconditions）](#Ri-expects)
* [I.7: 描述后置条件（State postconditions）](#Ri-post)
* [I.8: 优先使用`Ensures()`表示后置条件（Prefer `Ensures()` for expressing postconditions）](#Ri-ensures)
* [I.9: 使用概念说明模板接口的参数（If an interface is a template, document its parameters using concepts）](#Ri-concepts)
* [I.10: 使用异常表示任务执行失败（Use exceptions to signal a failure to perform a required task）](#Ri-except)
* [I.11: 永远不要转移裸指针`T*`或引用`T&`的所有权（Never transfer ownership by a raw pointer (`T*`) or reference (`T&`)）](#Ri-raw)
* [I.12: 使用`not_null`声明不可空指针（Declare a pointer that must not be null as `not_null`）](#Ri-nullptr)
* [I.13: 不要把数组传递为孤立指针（Do not pass an array as a single pointer）](#Ri-array)
* [I.22: 避免全局对象的复杂初始化（Avoid complex initialization of global objects）](#Ri-global-init)
* [I.23: 函数参数的个数尽量少（Keep the number of function arguments low）](#Ri-nargs)
* [I.24: 避免无关的同类型参数相邻（Avoid adjacent unrelated parameters of the same type）](#Ri-unrelated)
* [I.25: 优先使用抽象类作为类层次结构的接口（Prefer abstract classes as interfaces to class hierarchies）](#Ri-abstract)
* [I.26: 如果需要一个交叉编译器ABI，使用C-style子集（If you want a cross-compiler ABI, use a C-style subset）](#Ri-abi)
* [I.27: 对于稳定库的ABI，考虑Pimpl惯用法（For stable library ABI, consider the Pimpl idiom）](#Ri-pimpl)
* [I.30: 封装规则冲突（Encapsulate rule violations）](#Ri-encapsulate)

**另请参见（See also）**:

* [F: 函数（Functions）](#S-functions)
* [C.concrete: 具体类型（Concrete types）](#SS-concrete)
* [C.hier: 类层次结构（Class hierarchies）](#SS-hier)
* [C.over: 重载和重载运算符（Overloading and overloaded operators）](#SS-overload)
* [C.con: 容器和其他资源句柄（Containers and other resource handles）](#SS-containers)
* [E: 错误处理（Error handling）](#S-errors)
* [T: 模板和泛型编程（Templates and generic programming）](#S-templates)

### <a name="Ri-explicit"></a>I.1: 定义清晰明确的接口（Make interfaces explicit）

##### 原因（Reason）

正确性。在接口中没有明确陈述的假设，很容易被忽视且很难测试。

##### 不好的例子（Example, bad）

通过全局域或者名字空间域中的变量来隐含地控制一个函数的行为，容易引起潜在的混淆。例如：
...
如果两次调用`round(7.2)`很可能得出不同的结果，为什么会这样，对调用者而言并非显而易见。

##### 例外（Exception）

有时我们通过环境变量控制一组操作的细节，例如，设置标准与详细输出（normal vs. verbose），调试与优化（debug vs. optimized）。
使用非局部控制可能会造成混淆，不过控制其他固定语义的实现细节不存在问题。

##### 不好的例子（Example, bad）

通过非局部变量（例如，`errno`）的打印输出很容易被忽略。例如：

    // don't: no test of printf's return value
    fprintf(connection, "logging: %d %d %d\n", x, y, s);

如果connection断开导致没有日志输出产生怎么办呢？

**替代方案（Alternative）**: 抛出一个异常，异常不会被忽略。

**替代表述（Alternative formulation）**: 不要通过非局部方式或者隐含状态在接口上传递信息。
注意：non-`const`成员函数通过其对象状态将信息传递给其他成员函数。

**替代表述（Alternative formulation）**: 接口应该是一个函数或函数集。函数可以是模板函数，函数集可以是类或类模板。

##### 强制性（Enforcement）

* (简单) 函数不应该基于名字空间域中的变量值进行控制流判断。
* (简单) 函数不应改写名字空间域中的变量。

### <a name="Ri-global"></a>I.2: 避免non-`const`全局变量（Avoid non-`const` global variables）

##### 原因（Reason）

非`const`全局变量隐藏了依赖关系，使依赖关系容易发生不可预测的改变。
...
针对全局变量的规则也适用于名字空间域中的变量。

**替代方案（Alternative）**: 如果你希望使用全局域（或者名字空间域）中的数据来避免拷贝，考虑通过一个对象的`const`引用来传递数据。
另外一种解决方法是将数据定义为某个对象的状态，将操作定义为成员函数。

**警告（Warning）**: 提防数据竞争（data races）：如果一个线程可以访问非局部数据（或者通过引用传递的数据），同时另一个线程正在执行，此时可能发生数据竞争。
...
**参考文献（References）**: 参见 [调用函数的规则（rules for calling functions）](#SS-call).

##### 注解（Note）

本规则是“避免”，并非“不要使用”。因此，有很少的一些例外，如`cin`, `cout` 和 `cerr`等。

##### 强制性（Enforcement）

(简单) 报告在名字空间域声明的所有非`const`变量。

### <a name="Ri-singleton"></a>I.3: 避免单例（Avoid singletons）

##### 原因（Reason）

单例（Singleton）基本上就是伪装起来地复杂一些的全局对象。
...
如果你不想让全局变量发生改变，将其声明为 `const` 或者 `constexpr`。

##### 例外（Exception）

可以使用最简单的“单例”（简单到经常不把它当成一个单例）在初次使用时获得初始化（如果需要的话）：
...
这是解决初始化顺序相关问题的最有效的方法之一。
在多线程编程环境下，静态对象的初始化不会形成竞争条件（除非你不小心在构造器里面访问一个共享对象）。

注意：一个局部`static`对象的初始化不会形成竞争条件；但是，如果 `X` 的析构涉及同步操作，我们就必须采用稍微复杂一点的解决方法。
例如:
...
现在，必须有人以某种恰当的线程安全的方式 `delete` 那个对象。
这种处理方式当然非常容易出错，所以不要用那种技术，除非：

* `myX` 处于多线程代码中；
* `X` 对象需要被销毁（例如，因为要释放资源）；并且，`X` 的析构函数代码需要同步。

如果你也像其他许多人一样，把“单例”定义为只创建一个对象的类，那么像 `myX` 这样的函数就不是单例，这种有用的技术也就算不上是避免单例规则的例外了。
...
* 查找名字包含 `singleton` 的类。
* 查找只有一个对象创建的类（通过对象计数或者构造函数检查）。
* 如果一个类 X 具有一个public static 成员函数，其中包含一个类型为 X 的局部静态对象，并且返回静态对象的指针或引用，禁止这么用。

### <a name="Ri-typed"></a>I.4: 使接口准确并强类型化（Make interfaces precisely and strongly typed）

##### 原因（Reason）

类型是最简单、最好的文档，定义良好，可读性强，可以在编译期进行检查。
此外，类型准确的代码通常能更好的优化。
...
考虑如下代码:
    ...
    void pass(void* data);    // weak and under qualified type void* is suspicious（充满疑惑）

调用者不确定允许那些类型，是否data是可改变的，又没有指定 `const` 。
注意，所有指针类型都可以隐式地转换为void*，所以调用者随便提供一个指针值就可以了。
但是，被调用者（这里就是pass函数）就必须使用 `static_cast` 把data强制转换为一个不一定可行的类型然后使用之。
这种处理方式非常易于出错且代码冗长。

在设计阶段一些数据或许暂时不可描述，可以使用 `const void*` 传递数据。考虑使用 `variant` 或者基类指针来替代。

**替代方案（Alternative）**: 通常，可以通过把 `void*` 转换为 `T*` 或 `T&`，从而消除 `void*` 模板参数。
对于泛型代码， `T` 可以是一般模板参数的或概念约束的模板参数。
...
很明显调用者想描述的是一个矩形，但是参数与矩形那个部分相关联并不清楚。
另外，一个 `int` 可以表达任意信息，包括信息的任意单位，所以我们必须猜测四个 `int` 的含义。
很有可能，前两个 `int` 表示一个（ `x`,`y` ）坐标，但是其余两个 `int` 表示什么呢？

注释和参数名有助于解决问题，但是更好的方式是显式类型说明：
...
显然，通过静态类型系统不可能捕获所有错误。
（例如，第一个参数是矩形的top-left角点的假设，只能依靠惯例约定（命名和注释）来解决）。
...
参数类型及其值，没有传达出参数如何指定或者参数具有什么含义的信息。
...
对于一组布尔值的情况，请考虑使用一个标志枚举类型，这是表示一组布尔值的一种处理模式。

    enable_lamp_options(lamp_option::on | lamp_option::animate_state_transitions);

##### 不好的例子（Example, bad）

在如下的例子中，单从接口看不出来 `time_to_blink` 的含义是秒还是毫秒？

    void blink_led(int time_to_blink) // bad -- the unit is ambiguous（单位不清楚）
    {
        // ...
...
在C++11中引入的`std::chrono::duration` 类型有助于清晰的描述时间段的单位。
...
通过这种方式，可以把函数编写成能够接受任何时间段的单位。

    template<class rep, class period>  //rep是时间段的表示类型，可以是int
...
* (简单) 报告 `void*` 用作参数或返回类型的情况
* (简单) 报告多个 `bool` 参数的使用情况。
* (做好很难) 查找使用过多原始类型参数的函数。

### <a name="Ri-pre"></a>I.5: 描述前置条件（如果有）（State preconditions (if any)）

##### 原因（Reason）

参数具有用于约束其正确使用的含义。
实参在传递给函数的时候，必须满足关于函数参数的一系列预期假设，即前置条件。
...
这里 `x` 必须非负。类型系统很难自然地表示这种约束，所以我们必须使用其他手段。例如使用注释：

    double sqrt(double x); // x must be nonnegative

有些前置条件可以表示为断言（assertion）。例如：

    double sqrt(double x) { Expects(x >= 0); /* ... */ }

理想情况下，断言 `Expects(x >= 0)` 应该是 `sqrt()` 接口的一部分，当然这不容易实现。
所以，我们把断言置于函数定义中（函数体）。

**参考文献（References）**: `Expects()` 请参见 [GSL](#S-gsl).

##### 注解（Note）

优先使用一个形式化的需求规定，例如`Expects(p);`。
如果形式化需求规定不可行，使用文本注释，如 `// the sequence [p:q) is ordered using <`。

##### 注解（Note）

大部分成员函数都要求某些类不变量满足一个前置条件。
类不变量由构造函数建立，在每个成员函数的类外调用退出的时候必须重建。
我们不需要对每个成员函数都说明类不变量。

##### 强制性（Enforcement）

(不可强制实施，Not enforceable)

**另请参见（See also）**: 传递指针的规则（The rules for passing pointers. ???）

### <a name="Ri-expects"></a>I.6: 优先使用`Expects()`表示前置条件（Prefer `Expects()` for expressing preconditions）

##### 原因（Reason）

明确条件是前置的，并且能够进行支持工具的使用。
...
前置条件可以通过很多方式描述，包括注释、`if`语句和`assert()`。
但是，这些方式会导致很难将前置条件和普通代码区分开来，很难更新，很难被支持工具处理，并且可能有错误的语义
（我们总想在调试模式下尽量检查并中止出错程序，而在生产运行环境中不做这些检查）。

##### 注解（Note）

前置条件应该是接口的一部分，而非实现的一部分，但是目前C++语言还没有一种语言设施能够做到这一点。
未来一旦语言支持成为现实（例如，参见[契约提案（contract proposal）](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0380r1.pdf)），我们应采用标准版本的前置条件、后置条件和断言。

##### 注解（Note）

`Expects()` 也可以用于检查算法中间过程应该满足的条件。

##### 注解（Note）

使用 `unsigned` 回避[确保值非负（ensuring that a value is nonnegative）](#Res-nonnegative)问题不是一个好方法。

##### 强制性（Enforcement）

（不可强制实施，Not enforceable）找到前置条件断言的各种方式是不可行的。在缺少语言设施支持的前提下，对很容易标识的前置断言（`assert()`）进行警告没有多少价值。

### <a name="Ri-post"></a>I.7: 描述后置条件（State postconditions）

##### 原因（Reason）

检测结果错误的罪魁祸首，可能捕获错误的实现。
...
这里，我们不小心遗漏了前置条件说明，所以 height 和 width 必须是正数的约束没有显式说明。
此外，我们还遗漏了后置条件说明，如果`height * width`计算的面积超出最大整数范围，这个错误并非显而易见的，可能会发生溢出。
...
这里没有后置条件陈述 buffer 应该被清空的约束，因此优化器会删除明显冗余的 `memset()` 调用。
...
后置条件经常在函数的功能目标注释中进行非正式的陈述。 `Ensures()` 可以使后置条件的陈述更加系统、可见、可检查。

##### 注解（Note）

当返回结果不能直接反映某些约束或要求时，例如关于所使用的数据结构的某个状态，后置条件就非常重要了。

##### 示例（Example）

考虑如下操作一个 `Record` 的函数，其中使用了 `mutex` （互斥量）避免竞争条件：
...
这里，我们“忘记”陈述互斥量 `mutex` 应该被释放这个约束，因此我们不知道“没有确保互斥量 `mutex` 被释放”是一个缺陷还是一个有意设置的特性。
陈述后置条件可以使之清楚明了：

    void manipulate(Record& r)    // postcondition: m is unlocked upon exit（退出时m是unlocked）
    {
        m.lock();
        // ... no m.unlock() ...
    }

现在缺陷就很显然了（但是只是对于阅读注释的人类而言）。

还有更好的办法，使用 [资源获取即初始化（RAII）](#Rr-raii) 来保证后置条件（互斥锁必须被释放）在代码中强制执行：
...
理想情况下，后置条件在接口/声明中陈述以便用户容易看见。
仅仅与调用者相关的后置条件，应该在接口中陈述。
仅仅与内部状态相关的后置条件，应该隶属于定义/实现。

##### 强制性（Enforcement）

（不可强制执行）这是一条原理性的指导规则，一般情况下直接检查是不可行的。领域特定的检查器存在于许多工具链，如锁持有检查器（lock-holding
checkers）。

### <a name="Ri-ensures"></a>I.8: 优先使用`Ensures()`表示后置条件（Prefer `Ensures()` for expressing postconditions）

##### 原因（Reason）

明确条件是后置的，并且能够进行支持工具的使用。
...
后置条件可以通过很多方式描述，包括注释、`if`语句和`assert()`。
但是，这些方式会导致很难将后置条件和普通代码区分开来，很难更新，很难被支持工具处理，并且可能有错误的语义。

**替代方案（Alternative）**: “该资源必须被释放”的后置条件最好通过[资源获取即初始化（RAII）](#Rr-raii)来表示。

##### 注解（Note）

理想情况下， `Ensures` 应该是接口的一部分，但是这很不容易做到。
目前，我们是把 `Ensures` 置于定义（函数体）之中。 
未来一旦语言支持成为现实（例如，参见[契约提案（contract proposal）](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0380r1.pdf)），我们应采用标准版本的前置条件、后置条件和断言。

##### 强制性（Enforcement）

（不可强制实施，Not enforceable）找到后置条件断言的各种方式是不可行的。在缺少语言设施支持的前提下，对很容易标识的后置断言（`assert()`）进行警告没有多少价值。

### <a name="Ri-concepts"></a>I.9: 使用概念说明模板接口的参数（If an interface is a template, document its parameters using concepts）

##### 原因（Reason）

更准确地规定接口，在不远的将来可以进行编译期检查。

##### 示例（Example）

使用ISO概念技术规范（ISO Concepts TS）风格的需求规定。例如：
...
> TS，Technical Specification，技术规范，当前还没有未来很有可能包含进标准。

##### 注解（Note）

不久（或许2018年）大多数编译器将能够检查 `requires` 子句，只要移除 `//` 。
GCC 6.1之后会支持概念（concept）。

**另请参见（See also）**: [泛型编程（Generic programming）](#SS-GP) 和 [概念（concepts）](#SS-concepts)。

##### 强制性（Enforcement）

（暂时不可强制执行）相关语言设施正在制定规范。当语言设施可用之后，如果任何非可变模板参数不受概念约束（在其声明或 `requires` 子句中提及）就发出警告。

> 可变模板参数（variadic template parameter），用于处理模板参数个数的任意化。

### <a name="Ri-except"></a>I.10: 使用异常标识任务执行失败（Use exceptions to signal a failure to perform a required task）

##### 原因（Reason）

不能忽略错误，因为这会使系统或者计算处于一种未定义（或不期望）的状态。这是一个出错的主要源头。

##### 示例（Example）

    int printf(const char* ...);    // bad: return negative number if output fails（如果输出失败返回负数）

    template <class F, class ...Args>
    // good: throw system_error if unable to start the new thread（如果不能启动一个新线程，抛出system_error异常）
    explicit thread(F&& f, Args&&... args);

> 这里的 'printf' 和 'thread' 函数是ISO C++中的库函数。其中， 'printf' 在失败时返回负值， 'thread' 在失败时抛出system_error异常。
> 想要查看这两个函数的详细说明请参阅[C++语言参考](https://en.cppreference.com)。

##### 注解（Note）

什么是错误？

一个错误意味着，函数无法达到其宣称的目标（包括满足后置条件约束）。
调用忽略错误的代码会导致错误的结果或者未定义的系统状态。
例如，无法连接到远程服务器本身不是一个错误：服务器可以有各种理由拒绝一个连接，所以很自然地就应该返回一个用于调用者进行检查的结果。
然而，如果把不能建立连接当成一个错误，则失败就应该抛出一个异常。

##### 例外（Exception）

许多传统的接口函数（例如，UNIX信号处理程序）使用错误代码（例如， `errno`）来报告真正的状态代码，并没有使用错误。
对于这种情况我们并没有很好的替代方案，所以调用这些接口函数不算违反规则。

##### 替代方案（Alternative）

如果你不使用异常（例如，假设你的代码充斥着老式的裸指针或者有严格的实时约束），考虑使用返回值对的风格：
...
> std:tie，用于把多个左值引用打包成一个元组（tuple）结构。这里，使用std::tie把val和error_code打包成一个值对。

很不幸，这种处理风格会导致未初始化的变量问题。
在C++17中会提供一种处理该问题的语言设施[结构化绑定（structured bindings）](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0144r1.pdf)。
...
我们不认为“性能”是不使用异常的有效原因。

* 通常，显式的错误检查和处理的时空代价跟异常处理不相上下。
* 通常，使用异常的代码更干净，可以得到更好的性能（简化了程序的路径跟踪及其优化）。
* 对于性能关键代码，一个好的规则是将错误检查从性能关键代码中移出([检查（checking）](#Rper-checking))。
* 长期来看，更规范的代码可以得到更好的优化。
* 在提出性能要求之前总是仔细地进行[测量（measure）](#Rper-measure)。

**另请参见（See also）**: 报告前置条件和后置条件冲突，请参阅[I.5](#Ri-pre) and [I.7](#Ri-post)。

##### 强制性（Enforcement）

* （不可强制实施，Not enforceable）这条是原理性指导规则，不能直接进行检查。
* 查找 `errno`。

### <a name="Ri-raw"></a>I.11: 永远不要转移裸指针`T*`或引用`T&`的所有权（Never transfer ownership by a raw pointer (`T*`) or reference (`T&`)）

##### 原因（Reason）

如果不确定调用者或被调用者是否拥有对象，可能会发生泄漏或者提早析构。
...
谁来释放返回的 `X`？ 如果 `compute` 返回一个引用，则问题会更难以发现。
考虑按值（by value）返回结果（如果返回结果很大，使用move语义）：
...
**替代方案（Alternative）**: [传递所有权（Pass ownership）](#Rr-smartptrparam) 使用一个“智能指针”，如用于独占所有权的 `unique_ptr` 和用于共享所有权的 `shared_ptr`。
然而，这种替代方案不够优雅，通常比返回对象本身更低效，所以只有在需要引用语义时才使用智能指针。

**替代方案（Alternative）**: 有时候，一些老旧的代码无法修改，考虑到ABI兼容需求或者资源缺乏。
此时，使用[指南支持库（guidelines support library）](#S-gsl)中的 `owner` 来标记指针所有权：
...
这样就告诉分析工具： `res` 是所有者。也就是说，`res` 的值必须被`delete`或者转移到另一个所有者，就像这里`return`做的事情一样。

`owner` 在资源句柄的实现中可以类似地使用。

##### 注解（Note）

使用裸指针或者迭代器传递的每个对象，都假设被其调用者所拥有，因此对象的生命期由调用者处理。
从另一个角度观察：所有权转移API远少于指针传递API，故默认假设不会发生所有权转移。

**另请参见（See also）**: [参数传递（Argument passing）](#Rf-conventional)、[使用智能指针参数（use of smart pointer arguments）](#Rr-smartptrparam) 和 [值返回（value return）](#Rf-value-return).

##### 强制性（Enforcement）

* (简单) 在 `delete` 一个不是 `owner<T>` 的裸指针时，发出警告。建议使用标准库资源句柄处理方式或者使用 `owner<T>`。
* (简单) 在每个代码路径上如果不能重置（ `reset` ）或者显式 `delete` 一个 `owner` 指针时，发出警告。
* (简单) 如果 `new` 的返回值或者一个返回 `owner` 的函数调用，被赋予一个裸指针或非 `owner` 引用时，发出警告。

### <a name="Ri-nullptr"></a>I.12: 使用`not_null`声明不可空指针（Declare a pointer that must not be null as `not_null`）

##### 原因（Reason）

有助于避免解引用 `nullptr` 的错误。
避免对 `nullptr` 进行冗余检查以提升性能。

##### 示例（Example）

    int length(const char* p);            // it is not clear whether length(nullptr) is valid（是否计算空指针的length是有效的）
...
通过在代码中陈述意图，实现者和支持工具可以提供更好的诊断，如通过静态分析发现某些错误类；可以进行优化，如移除分支和null测试。

##### 注解（Note）

`not_null` 定义在[指南支持库（guidelines support library）](#S-gsl)。

##### 注解（Note）

假设`char` 指针会指向一个C-style字符串（以0结尾的字符串）不过是一个隐含的约定，这是一个潜在的混乱和错误之源。
优先使用 `czstring` 替代 `const char*`。

    // we can assume that p cannot be nullptr
    // we can assume that p points to a zero-terminated array of characters
    int length(not_null<zstring> p);

注意: `length()` 不过是伪装的 `std::strlen()`。

##### 强制性（Enforcement）

* (简单)((基础)) 如果一个函数，在所有控制路径上访问指针参数之前都检查其是否非 `nullptr`，则发出警告：它应该声明为 `not_null`。 
* (复杂) 如果返回指针的函数，在其所有返回路径上都确保该指针非 `nullptr`，则发出警告：返回类型应该声明为 `not_null`。 

### <a name="Ri-array"></a>I.13: 不要把数组传递为孤立指针（Do not pass an array as a single pointer）

##### 原因（Reason）

 用(pointer, size)-style表示数组参数的接口易于出错。此外，一个指向数组的普通指针，必须依赖于某些约定才能够由被调用者确定数组的空间大小。
...
如果`q`指向的数组的元素少于`n`，该怎么办？ 此时，我们会覆盖一些可能无关的内存。
如果`p`指向的数组的元素少于`n`，该怎么办？ 此时，我们会读取一些可能无关的内存。
这两种情况都是未定义的行为（undefined behavior），是一个非常令人讨厌的潜在Bug。
...
把 `10` 传递给参数 `n` 可能发生错误：最常用的惯例约定是假设下标是`[0:n)`，但没有明确说明。
更坏的情况是 `draw()` 的调用会被编译成：有一个从数组到指针的隐式转换（数组退化）和另一个从 `Circle` 到 `Shape` 的隐式转换。
`draw()` 函数根本无法安全地遍历该数组：因为它没有办法知道数组元素的大小。

**替代方案（Alternative）**: 使用支持类来确保元素个数是正确的，并防止危险的隐式转换。例如：

    void draw2(span<Circle>);
    Circle arr[10];
    // ...
    draw2(span<Circle>(arr));  // deduce the number of elements（推断元素个数）
    draw2(arr);    // deduce the element type and array size（推断元素类型和个数）

    void draw3(span<Shape>);
    draw3(arr);    // error: cannot convert Circle[10] to span<Shape>

 `draw2()` 函数传递了与 `draw()` 函数一样的信息量，但是 `draw2()` 使假设参数是一个`Circle`范围这一事实变得更为清晰明确。参见 ???。

##### 例外（Exception）

使用 `zstring` 和 `czstring` 表示一个C语言风格的zero-terminated字符串。这样做时，使用[GSL](#GSL)中的 `string_span` 阻止发生范围错误（range errors）。

##### 强制性（Enforcement）

* (简单) ((有限范围)) 对于任何依赖于数组到指针隐式转换的表达式，发出警告。对于zstring/czstring指针类型允许例外。
* (简单) ((有限范围)) 对于求值结果产生指针的任意指针算术运算表达式，发出警告。对于zstring/czstring指针类型允许例外。

### <a name="Ri-global-init"></a>I.22: 避免全局对象的复杂初始化（Avoid complex initialization of global objects）

##### 原因（Reason）

全局对象的复杂初始化会导致未定义的执行顺序。
...
由于 `x` 和 `y` 在不同的编译单元中， `f()` 和 `g()`的调用顺序是未定义的，可能导致访问未初始化的 `const`。
这表明全局域（名字空间域）对象的初始化顺序问题，不仅仅决定于全局变量。

##### 注解（Note）

在并发代码中初始化顺序问题处理起来尤其困难。通常，最好是完全避免全局域（名字空间域）对象。

##### 强制性（Enforcement）

* 对调用非 `constexpr` 函数的全局域初始式进行标记。
* 对访问 `extern` 对象的全局域初始式进行标记。

### <a name="Ri-nargs"></a>I.23: 函数参数的个数尽量少（Keep the number of function arguments low）

##### 原因（Reason）

参数太多容易引发混乱。传递过多参数比起替代方案来通常代价更高。
...
1. *缺少抽象*
   缺少抽象，导致复合值被传递为多个单一元素，而非作为一个独立对象进行传递从而保证某种强制不变性。
   这种方式不仅仅会导致参数列表膨胀，更重要地是会导致错误，因为组成元素之不再受某种强制不变性保护。
...
原因是上面的问题1——缺少抽象。STL传递迭代器对（未封装的组件值），而不是传递一个抽象的范围（range）。

这里，我们有四个模板参数和六个函数参数。
为了简化最常用和最简单的使用情况，比较参数 `comp` 可以缺省为 `<` ：
...
当然，这并没有降低总体的复杂性，但是至少表明上约减了呈现给用户的复杂性。
要想真正约减掉参数个数，我们需要把参数包装成更高层次的抽象：

    template<class InputRange1, class InputRange2, class OutputIterator>
    OutputIterator merge(InputRange1 r1, InputRange2 r2, OutputIterator result);

把参数组织成“包裹（bundles）”是一种通用技术，用于约减参数个数和增加检查机会。

或者，我们可以使用概念（concept，定义在ISO TS技术规范中）来定义 `merge()` 三个参数类型的意图。
...
这里，使用抽象具有安全性和健壮性的好处，自然也减少了参数个数。

##### 注解（Note）

多少参数算过多？通常不要超过4个参数。
有些函数最好用四个独立参数来表达，这不算过多。

**替代方案（Alternative）**: 使用更好的抽象：把多个参数组织成有意义的对象，然后传递对象（by value 或者 by reference）。

**替代方案（Alternative）**: 使用默认参数，或者重载最常用的参数更少的调用形式。

##### 强制性（Enforcement）

* 当一个函数声明了两个同类型的迭代器（包括指针）而没有用range或view时，发出警告。
* (不可强制实施）这是一条原理性规则，不能直接检查。

### <a name="Ri-unrelated"></a>I.24: 避免无关的同类型参数相邻（Avoid adjacent unrelated parameters of the same type）
...
这是一个非常让人讨厌的 K&R C-style 风格的接口，极容易颠倒 "to" 和 "from" 参数。
...
如果参数的顺序无关紧要，本规则就不是问题：

    int max(int a, int b);

##### 替代方案（Alternative）

不要把数组传递为指针，传递一个表示范围的对象（例如，一个 `span`）：

    void copy_n(span<const T> p, span<T> q);  // copy from p to q

##### 替代方案（Alternative）

定义一个 `struct` 作为参数类型，对相应的参数字段进行命名：
...
这种处理会使未来的读者清楚地了解函数的调用方法，只需要在调用点按名字填充参数即可。

##### 强制性（Enforcement）

(简单) 如果两个相邻参数具有相同类型，发出警告。

### <a name="Ri-abstract"></a>I.25: 优先使用抽象类作为类层次结构的接口（Prefer abstract classes as interfaces to class hierarchies）
...
你肯定知道 `Shape` 会在某个地方被重新拎出来 :-)
 ...
 `Shape` 会强迫每个派生类计算一个center中心，即使对于派生类而言center没有意义且从来不会用到。
类似的，不是每个 `Shape` 都有一个 `Color`，许多 `Shape` 并不适合用 `Point` 序列表示其outline轮廓。
抽象类被发明，用于阻止用户编写这样的类：
...
(简单) 如果一个类 `C` 的指针或引用被赋值给基类指针或引用，且基类包含数据成员，发出警告。

### <a name="Ri-abi"></a>I.26: 如果需要一个交叉编译器ABI，使用C-style子集（If you want a cross-compiler ABI, use a C-style subset）

##### 原因（Reason）

对于类、异常处理、函数名及其他实现细节，不同的编译器会以不同的二进制布局来实现。

##### 例外（Exception）

你可以仔细设计一个接口，使用一些精心挑选的高层C++类型。参见 ???。

##### 例外（Exception）

在某些平台上存在常见的ABI，可以使你从这些严苛的限制中解放出来。

##### 注解（Note）

如果你使用单一的编译器，则可以在接口中使用C++全集。在升级到新的编译器版本时需要重新编译。

##### 强制性（Enforcement）

(不可强制实施) 很难可靠地识别出在哪里一个接口构成一个ABI的一部分。

### <a name="Ri-pimpl"></a>I.27: 对于稳定库的ABI，考虑Pimpl惯用法（For stable library ABI, consider the Pimpl idiom）

##### 原因（Reason）

由于私有数据成员参与类布局并且私有成员函数参与重载解析，因此对这些实现细节的更改需要类的所有用户进行重新编译。
一个非多态接口类持有指向实现的指针（Pimpl），通过间接的方式，可以将类的用户与其实现的变化隔离开来。
...
与此惯用法相关的衡量和其他实现细节，参见 [GOTW #100](https://herbsutter.com/gotw/_100/) 和 [cppreference](http://en.cppreference.com/w/cpp/language/pimpl)。

##### 强制性（Enforcement）

(不可强制实施) 很难可靠地识别出在哪里一个接口构成ABI的一部分。

### <a name="Ri-encapsulate"></a>I.30: 封装规则冲突（Encapsulate rule violations）

##### 原因（Reason）

保持代码简洁、安全。
有时候考虑到逻辑或性能原因，一些丑陋、不安全或者易出错的技术可能是必要的。
如果这样，那就把他们限制在一个局部范围，而非“感染”其他接口，广大程序员朋友必须意识到这种微妙之处。
如果可能的话，实现的复杂性不应该通过接口暴露传播到用户代码中。

##### 示例（Example）

考虑一个程序，依赖于某种形式的输入（例如， `main` 的参数），可能使用文件输入、命令行输入或者标准输入。
我们可能这样编写代码：
...
这种处理方式违反了规则：[禁止未初始化变量（against uninitialized variables）](#Res-always)，[禁止忽略所有权（against ignoring ownership）](#Ri-raw)，[禁止魔术常量（against magic constants）](#Res-magic)。
特别地，我们还要记住在某个地方必须 `delete` 指针inp:

    if (owned) delete inp;

通过使用 `unique_ptr` （带有一个特殊删除器deleter，对 `cin` 无操作），我们可以处理这个特殊例子。
但是，这种处理方式对于初学者而言过于复杂。
此外，这个例子其实对应于一个更一般的问题，我们认为静态的属性（这里是所有权）很少需要在运行期解决。

常见的、最经常出现的、最安全的案例可以静态处理，所以我们不想增加它们的代价和复杂性。
但是我们还必须处理那些不常见的、不太安全的、代价更高昂的情况，这样一些案例参见[[Str15]](http://www.stroustrup.com/resource-model.pdf)。
...
现在， `istream` 所有权的动态特性被封装起来。很可能在实际代码中还会增加一点潜在错误的检查。

##### 强制性（Enforcement）

* 困难。很难确定哪些破坏规则的代码是必不可少的。
* 如果禁止规则使得违规行为跨越接口，标记之。

# <a name="S-functions"></a>F: 函数（Functions）

函数，规定一个使系统从一个一致状态迁移到下一个一致状态的行为或者计算。函数是程序最基础的构成要素（building block）。

应该尽可能给函数取一个有意义的名字，规格化说明参数的需求，清楚地陈述参数与结果之间的关系。
函数的实现不是规格说明，注意区分函数“做什么”和“怎么做”。
在大多数接口中，函数是最关键的部分，所以仔细体会关于接口的规则。

函数规则概要：

函数定义规则（Function definition rules）：

* [F.1: 将有意义的业务包装成精心命名的函数（"Package" meaningful operations as carefully named functions）](#Rf-package)
* [F.2: 单一函数应执行单一逻辑业务（A function should perform a single logical operation）](#Rf-logical)
* [F.3: 使函数短小简单（Keep functions short and simple）](#Rf-single)
* [F.4: 将编译期求值的函数声明为 `constexpr` （If a function may have to be evaluated at compile time, declare it `constexpr`）](#Rf-constexpr)
* [F.5: 将极小且时间关键的函数声明为inline（If a function is very small and time-critical, declare it inline）](#Rf-inline)
* [F.6: 将没有异常抛出的函数声明为 `noexcept` （If your function may not throw, declare it `noexcept`）](#Rf-noexcept)
* [F.7: 一般用途函数的参数避免智能指针，使用 `T*` 或 `T&`（For general use, take `T*` or `T&` arguments rather than smart pointers）](#Rf-smart)
* [F.8: 优先使用纯函数（Prefer pure functions）](#Rf-pure)
* [F.9: 无用参数不要命名（Unused parameters should be unnamed）](#Rf-unused)

参数传递表达式规则（Parameter passing expression rules）:

* [F.15: 优先使用简单且约定俗成的方式传递信息（Prefer simple and conventional ways of passing information）](#Rf-conventional)
* [F.16: 对于"in"参数，拷贝廉价的类型按值传递，其他按 `const` 引用传递（For "in" parameters, pass cheaply-copied types by value and others by reference to `const`）](#Rf-in)
* [F.17: 对于"in-out"参数，按非`const`引用传递（For "in-out" parameters, pass by reference to non-`const`）](#Rf-inout)
* [F.18: 对于"will-move-from"参数，按右值引用 `X&&` 传递参数并使用 `std::move` 转换参数（For "will-move-from" parameters, pass by `X&&` and `std::move` the parameter）](#Rf-consume)
* [F.19: 对于"forward"参数，按右值引用 `TP&&` 传递参数且只用 `std::forward` 转换参数（For "forward" parameters, pass by `TP&&` and only `std::forward` the parameter）](#Rf-forward)
* [F.20: 对于"out"输出参数，优先使用返回值而非"out"参数（For "out" output values, prefer return values to output parameters）](#Rf-out)
* [F.21: 对于返回多个"out"值，优先返回一个结构或元组（To return multiple "out" values, prefer returning a struct or tuple）](#Rf-out-multi)
* [F.60: 如果无参数是有效选项，优先使用 `T*` 而非 `T&`（Prefer `T*` over `T&` when "no argument" is a valid option）](#Rf-ptr-ref)

参数传递语义规则（Parameter passing semantic rules）：

* [F.22: 使用 `T*` 或 `owner<T*>` 来指定单一对象（Use `T*` or `owner<T*>` to designate a single object）](#Rf-ptr)
* [F.23: 使用 `not_null<T>` 指示"null"不是有效值（Use a `not_null<T>` to indicate that "null" is not a valid value）](#Rf-nullptr)
* [F.24: 使用 `span<T>` 或 `span_p<T>` 指定一个半开序列（Use a `span<T>` or a `span_p<T>` to designate a half-open sequence）](#Rf-range)
* [F.25: 使用 `zstring` 或 `not_null<zstring>` 指定一个C风格的字符串（Use a `zstring` or a `not_null<zstring>` to designate a C-style string）](#Rf-zstring)
* [F.26: 使用 `unique_ptr<T>` 来传递指针所有权（Use a `unique_ptr<T>` to transfer ownership where a pointer is needed）](#Rf-unique_ptr)
* [F.27: 使用 `shared_ptr<T>` 来共享所有权（Use a `shared_ptr<T>` to share ownership）](#Rf-shared_ptr)

<a name="Rf-value-return"></a>值返回语义规则（Value return semantic rules）:

* [F.42: 返回 `T*` 来指示一个位置（only）（Return a `T*` to indicate a position (only)）](#Rf-return-ptr)
* [F.43: 永远不要直接或间接地返回局部对象的指针或引用（Never (directly or indirectly) return a pointer or a reference to a local object）](#Rf-dangle)
* [F.44: 要返回对象但不希望拷贝时返回一个 `T&` （Return a `T&` when copy is undesirable and "returning no object" isn't needed）](#Rf-return-ref)
* [F.45: 不要返回右值引用 `T&&`（Don't return a `T&&`）](#Rf-return-ref-ref)
* [F.46:  `main()`的返回值是`int`（`int` is the return type for `main()`）](#Rf-main)
* [F.47: 赋值操作符返回 `T&` （Return `T&` from assignment operators）](#Rf-assignment-op)
* [F.48: 不要返回`return std::move(local)`（Don't `return std::move(local)`）](#Rf-return-move-local)

其他函数规则（Other function rules）：

* [F.50: 如果不想用函数就使用一个lambda函数（捕获局部变量或者编写局部函数）（Use a lambda when a function won't do (to capture local variables, or to write a local function)）](#Rf-capture-vs-overload)
* [F.51: 如果可以选择，优选默认参数胜过重载（Where there is a choice, prefer default arguments over overloading）](#Rf-default-args)
* [F.52: 在局部使用及传递给算法的lambda函数中优先以引用方式捕获（Prefer capturing by reference in lambdas that will be used locally, including passed to algorithms）](#Rf-reference-capture)
* [F.53: 在非局部使用、包含返回、或者传递给另一个线程的lambda函数中避免以引用方式捕获（Avoid capturing by reference in lambdas that will be used nonlocally, including returned, stored on the heap, or passed to another thread）](#Rf-value-capture)
* [F.54: 如果捕获 `this` 指针，则显式地捕获所有变量（非默认捕获）（If you capture `this`, capture all variables explicitly (no default capture)）](#Rf-this-capture)
* [F.55: 不要使用 `va_arg` 参数（Don't use `va_arg` arguments）](#F-varargs)

函数与lambda函数和函数对象非常相似。

**另请参见（See also）**: [C.lambdas: 函数对象和lambda函数（Function objects and lambdas）](#SS-lambdas)

## <a name="SS-fct-def"></a>F.def: 函数定义（Function definitions）

函数定义是规定了函数实现（函数体）的函数声明。

### <a name="Rf-package"></a>F.1: 将有意义的业务包装成精心命名的函数（"Package" meaningful operations as carefully named functions）

##### 原因（Reason）

函数分解出了公共代码，从而使得代码更具可读性、更可能重用；通过函数可以将复杂代码中的错误限制于局部。
对于一个规格化良好（well-specified）的行为，把它从代码中分离出来并赋予一个名字。
...
##### （禁止的例子）Example, don't
 ...
 `read_and_print` 函数几乎一切都是错误的。它读取数据，然后写数据（到一个固定的 `ostream`），写错误消息（到一个固定的 `ostream`），仅处理一个 `int`。
 `read_and_print` 函数不可能重用，逻辑分离的操作被混杂的一起，局部变量在逻辑使用结束之后依然可见（in scope）。
对于一个很小的例子，看起来似乎OK；但是如果输入操作、输出操作和错误处理更为复杂，则这种混杂起来的情况会变得很难理解。

##### 注解（Note）

如果你编写了一个非平凡的lambda函数，可能在多个地方使用，通过将其赋予一个变量（通常非局部变量）的方式给lambda函数命名。

##### 示例（Example）

    sort(a, b, [](T x, T y) { return x.rank() < y.rank() && x.value() < y.value(); });

通过给lambda函数命名，可以将表达式分解成不同的逻辑部分，并且对lambda函数的含义给予强烈的暗示。
...
对于性能和可维护性而言，最短的代码可能并非最好的。

##### 例外（Exception）

循环体，包括作为循环体使用的lambda函数，很少需要命名。当然，循环体如果很大（例如，几十行或几十页）可能会有问题。
规则[使函数短小简单（Keep functions short and simple）](#Rf-single)要求：“使循环体短小精悍”。
类似的，用作回调参数的lambda函数有时候不是平凡的，但不可能被重用。

##### 强制性（Enforcement）

* 参见 [使函数短小简单（Keep functions short and simple）](#Rf-single)。
* 将在不同地方使用的相同和非常相似的lambda函数标记出来。

### <a name="Rf-logical"></a>F.2: 单一函数应执行单一逻辑业务（A function should perform a single logical operation）

##### 原因（Reason）

执行单一业务的函数，更易于理解、测试和重用。
...
这是一个与特定输入输出("cin", "cout")紧密捆绑的整体，永远不会存在重用的机会。
考虑经函数分解成合适的逻辑部分并参数化（parameterize）：
...
现在，在需要的时候可以把 `read` 和 `print` 组合起来定义 `read_and_print`：
...
如果必要，我们还可以进一步对 `read()` 和 `print()` 的数据类型、I/O机制、错误响应等进行模板化。例如：
...
* 如果函数具有多个 "out" 参数化，标记为可疑。使用返回值替代，包括使用 `tuple` 处理多个返回值。
* 如果函数很大在一个编辑屏幕上放不下，标记为可疑。考虑将该函数分解为多个更小的、命名良好的子任务。
* 如果函数具有七个或更多的参数，标记为可疑。
* 
### <a name="Rf-single"></a>F.3: 使函数短小简单（Keep functions short and simple）

##### 原因（Reason）

大函数很难阅读，更可能包含复杂的代码，更可能出现变量作用域超出所需最小作用域的情况。
具有复杂控制结构的函数，往往很长，更有可能隐藏逻辑错误。
...
`simple_func`函数过于复杂，怎么知道是否所有可能选项都已经被正确的处理了？而且，`simple_func`函数还违反了其他一些规则。

我们可以这样重构`simple_func`函数：
...
“一屏放不下”通常是“函数太大”的一个很好的可操作定义。
1~5行的函数（One-to-five-line functions）应该认为是合适的。

##### 注解（Note）

将大型函数分解成更小的、内聚的、恰当命名的函数。
在函数调用的代价很重要的场合，小型简单函数更易于实现inline。

##### 强制性（Enforcement）

* 把不能“一屏容纳”的函数标记出来。屏幕有多大呢？考虑60行*140字符，这大概是一页书可以合理容纳的最大尺寸。
* 把过于复杂的函数标记出来。多复杂算是复杂呢？可以使用圈复杂度（cyclomatic complexity），考虑超过10个逻辑路径就是复杂，一个简单分支计为一个路径。

### <a name="Rf-constexpr"></a>F.4: 将编译期求值的函数声明为 `constexpr` (If a function may have to be evaluated at compile time, declare it `constexpr`)

##### 原因（Reason）

 `constexpr` 用于告诉编译器：允许编译期求值。
...
`constexpr` 不能保证编译期求值。
它仅仅保证：对于参数是常量表达式的函数调用，如果程序员要求或者编译器优化要求，会在编译期进行函数求值。

    constexpr int min(int x, int y) { return x < y ? x : y; }

    void test(int v)
    {
        int m1 = min(-1, 2);            // probably compile-time evaluation（可能会编译期求值）
        constexpr int m2 = min(-1, 2);  // compile-time evaluation（编译期求值）
        int m3 = min(-1, v);            // run-time evaluation（运行期求值）
        constexpr int m4 = min(-1, v);  // error: cannot evaluate at compile time（错误，无法进行编译期求值）
    }

##### 注解（Note）

不要试图把所有函数都做成 `constexpr`，大多数计算适合在运行期完成。

##### 注解（Note）

任何API，如果最终需要依赖于高层的运行期配置或者业务逻辑，不要声明为 `constexpr`。
编译器无法对这种API进行求值，任何依赖于这种API的 `constexpr` 函数都必须进行重构或删除 `constexpr` 。

##### 强制性（Enforcement）

不可能且没有必要。
在要求一个常量的时候却调用了一个非 `constexpr` 函数，编译器会给出错误。

### <a name="Rf-inline"></a>F.5: 将极小且时间关键的函数声明为 `inline` （If a function is very small and time-critical, declare it `inline`）

##### 原因（Reason）

有些编译优化器无需程序员的提示就可以很好的处理内联，但是我们不能依赖于编译优化器的能力。
我算了一下，大约在过去40年的时间里，我们一直承诺编译器可以在没有人类提示的情况下更好的进行内联处理。
今天，我们依然在等待奇迹出现。
指定 `inline` 能够让编译器更好的进行内联处理工作。
...
不要把 `inline` 函数放到一个稳定性要求很高的接口里面，除非你很确定它不会改变。
内联函数是ABI的一部分。

##### 注解（Note）

`constexpr` 隐含地指定了 `inline`。
...
模板函数（包括模板成员函数）通常定义在头文件中，因此内联。

##### 强制性（Enforcement）

如果一个 `inline` 函数超过三行语句且可以非内联声明（如类成员函数），标记出来。

> out-ot-line vs. inline

### <a name="Rf-noexcept"></a>F.6: 将没有异常抛出的函数声明为 `noexcept` （If your function may not throw, declare it `noexcept`）

##### 原因（Reason）

如果不抛出异常，则认为程序不应处理错误，而应尽可能快地终止。
将一个函数声明为 `noexcept` ，可以通过减少可能执行路径的数量来帮助优化器。此外，还能在执行失败时加速退出程序。

##### 示例（Example）

如果一个函数完全使用C语言或者其他没有异常处理机制的语言编写，声明 `noexcept` 。
C++标准库隐式地将C标准库中的所有函数声明为 `noexcept` 。

##### 注解（Note）

`constexpr` 函数在运行期求值时可以抛出异常，所以有时候需要 `noexcept` 。

##### 示例（Example）

甚至可以在可能抛出异常的函数上使用 `noexcept` 。
...
如果 `collect()` 运行时导致内存耗尽，程序就崩溃了。
除非我们进行了精心设计以保证在内存耗尽时程序依然存活，否则将 `collect()` 声明为 `noexcept`其实没有什么问题。
内存耗尽之后很难做出任何聪明的补救措施，`terminate()` 函数可能会生成合适的错误日志信息。

##### 注解（Note）

在决定是否要将一个函数标记为 `noexcept` 的时候（尤其是考虑异常抛出和内存分配问题时），你必须了解代码运行的执行环境。
想要实现完美通用的代码（如标准库和其他类似的实用工具代码），需要支持在运行环境下能够恰当地处理 `bad_alloc` 异常。
然而，大多数程序和执行环境并不能有价值地处理内存分配失败，并且在这些情况下中止程序是对分配失败最干净、最简单的响应。
如果知道你的程序代码不需要响应内存分配失败，那么即使函数有分配内存的操作，我们也可以给它加上 `noexcept` 。

换句话说：程序中的多数函数都会抛出内存分配异常（例如，由于使用 `new`、调用使用 `new` 的函数、或者使用了通过抛出异常报告失败的库函数），所以不要随意到处使用 `noexcept`而不考虑处理可能的异常。

对于经常使用的低层函数，`noexcept` 可能是最有用且最清晰正确的。

##### 注解（Note）

析构函数（destructor）、 `swap`函数、 move操作、 默认构造函数永远不要抛出异常。

##### 强制性（Enforcement）

* 如果函数没有 `noexcept`实际上却不能抛出异常，标记出来。
* 如果  `swap`、 `move`、 析构函数、 默认构造函数抛出异常，标记出来。

### <a name="Rf-smart"></a>F.7: 一般用途函数的参数避免智能指针，使用 `T*` 或 `T&`（For general use, take `T*` or `T&` arguments rather than smart pointers）

##### 原因（Reason）

传递一个智能指针会转移或或共享所有权，应该仅用在需要所有权语义时（参见 [R.30](#Rr-smartptrparam)）。
智能指针作为参数传递，会限制函数的使用，其调用者只能使用智能指针。
传递一个共享的智能指针（例如，`std::shared_ptr`）隐含着需要付出运行期代价。

##### 示例（Example）

    // accepts any int*（接受任何整型指针）
    void f(int*);

    // can only accept ints for which you want to transfer ownership（只能接受要转让所有权的int）
    void g(unique_ptr<int>);

    // can only accept ints for which you are willing to share ownership（只能接受要分享所有权的int）
    void g(shared_ptr<int>);

    // doesn't change ownership, but requires a particular ownership of the caller（不改变所有权，但需要调用者具有特定所有权）
    void h(const unique_ptr<int>&);

    // accepts any int（接受任何int）
    void h(int&);
...
进一步参见 [R.30](#Rr-smartptrparam).

##### 注解（Note）

孤悬指针能够静态地捕获，所以我们不必依赖于资源管理来避免来自孤悬指针的违规行为。

**另请参见（See also）**:

* [如果无参数是有效选项，优先使用 `T*` 而非 `T&`（Prefer `T*` over `T&` when "no argument" is a valid option）](#Rf-ptr-ref)
* [智能指针概要（Smart pointer rule summary）](#Rr-summary-smartptrs)

##### 强制性（Enforcement）

标记一个智能指针类型（一个重载了 `operator->` 或 `operator *` 的类型）的参数，如果它不使用所有权语义；指的是：
*可复制但从未复制或移动过，或可移动但从未移动过；
*并且，永远不会被修改或传递给另一个可以这样做的函数。

### <a name="Rf-pure"></a>F.8: 优先使用纯函数（Prefer pure functions）

##### 原因（Reason）

纯函数易于推理，易于优化（甚至并行），易于记忆。

> 纯函数（pure function），返回结果只依赖于它的参数，并且在执行过程没有副作用，例如sin、cos等算术运算函数。
...
### <a name="Rf-unused"></a>F.9: 无用参数不要命名（Unused parameters should be unnamed）

##### 原因（Reason）

可读性（Readability），禁止无用参数的警告。
...
在1980年初引入了允许参数未命名的语言特性来解决这个问题。

##### Enforcement

标记命名的无用参数。

## <a name="SS-call"></a>F.call: 参数传递（Parameter passing）

有各种各样的方式可以用于传递参数给函数和返回值。

### <a name="Rf-conventional"></a>F.15: 优先使用简单且约定俗成的方式传递信息（Prefer simple and conventional ways of passing information）

##### 原因（Reason）

使用“不寻常和聪明”的技术往往出人意料，其他程序员理解起来比较困难，也容易导致缺陷。
如果确实感觉需要超越常用技术进行优化，评估一下以确保性能改进，并且需要保证文档/注释，因为性能改进往往伴随着不可移植。

在后续指南中总结了参数传递的建议，F.16-21.

常用的参数传递方法（Normal parameter passing）：

![常用的参数传递方法表（Normal parameter passing table）](./param-passing-normal.png "Normal parameter passing")

高级的参数传递方法（Advanced parameter passing）：

![高级的参数传递方法表（Advanced parameter passing table）](./param-passing-advanced.png "Advanced parameter passing")

仅在证明的确需要之后才使用高级技术，并在注释中对需求进行文档说明。

### <a name="Rf-in"></a>F.16: 对于"in"参数，拷贝廉价的类型按值传递，其他按 `const` 引用传递（For "in" parameters, pass cheaply-copied types by value and others by reference to `const`）

##### 原因（Reason）

使调用者知道函数不会修改参数，且允许以右值初始化。

什么是“拷贝廉价（cheap to copy）”，依赖于机器的体系结构，但是两个或三个字长（double、pointer、reference）通常按值传递更好。
如果拷贝代价低廉，拷贝的简单性和安全性就更值得关注。对于小型对象（最多两到三个字长），按值传递比按引用传递更快，因为按引用传递需要函数进行一个额外的间接访问。
...
过去曾经有一些指导规则，要求赋值操作符返回 `const T&`。
这主要是为了避免形如 `(a = b) = c` 的代码。这样的代码并不常见，不足以成为违反标准类型一致性的凭据。
...
* Bjarne Stroustrup: [C++11聽Style](http://channel9.msdn.com/Events/GoingNative/GoingNative-2012/Keynote-Bjarne-Stroustrup-Cpp11-Style). 2012.
* Bjarne Stroustrup: [The Essence of C++: With Examples in C++84, C++98, C++11, and聽C++14](http://channel9.msdn.com/Events/GoingNative/2013/Opening-Keynote-Bjarne-Stroustrup). 2013
...
[\[Cline99\]](#Cline99) 搂22.03-11, [\[Dewhurst03\]](#Dewhurst03) 搂52-53, [\[Koenig97\]](#Koenig97) 搂4, [\[Lakos96\]](#Lakos96) 搂10.3.5, [\[Meyers97\]](#Meyers97) 搂13, [\[Murray93\]](#Murray93) 搂2.1.3, [\[Sutter00\]](#Sutter00) 搂47
...
**References**: [\[Alexandrescu01\]](#Alexandrescu01) 搂3, [\[Boost\]](#Boost), [\[Dewhurst03\]](#Dewhurst03) 搂75, [\[Meyers97\]](#Meyers97) 搂46, [\[Stroustrup00\]](#Stroustrup00) 搂15.4.3, [\[Taligent94\]](#Taligent94)
...
**References**: [\[C++CS\]](#CplusplusCS) Item 50, [\[Cargill92\]](#Cargill92) pp. 77-79, 207, [\[Cline99\]](#Cline99) 搂21.06, 21.12-13, [\[Henricson97\]](#Henricson97) pp. 110-114, [\[Koenig97\]](#Koenig97) Chapters 4, 11, [\[Meyers97\]](#Meyers97) 搂14, [\[Stroustrup00\]](#Stroustrup00) 搂12.4.2, [\[Sutter02\]](#Sutter02) 搂27, [\[Sutter04\]](#Sutter04) 搂18
...
> If a destructor called during stack unwinding exits with an exception, terminate is called (15.5.1). So destructors should generally catch exceptions and not let them propagate out of the destructor. --[\[C++03\]](#Cplusplus03) 搂15.2(3)
>
> No destructor operation defined in the C++ Standard Library (including the destructor of any type that is used to instantiate a standard-library template) will throw an exception. --[\[C++03\]](#Cplusplus03) 搂17.4.4.8(3)
...
**References**: [\[C++CS\]](#CplusplusCS) Item 51; [\[C++03\]](#Cplusplus03) 搂15.2(3), 搂17.4.4.8(3), [\[Meyers96\]](#Meyers96) 搂11, [\[Stroustrup00\]](#Stroustrup00) 搂14.4.7, 搂E.2-4, [\[Sutter00\]](#Sutter00) 搂8, 搂16, [\[Sutter02\]](#Sutter02) 搂18-19
...
**References**: [\[C++CS\]](#CplusplusCS) Item 52; [\[Cline99\]](#Cline99) 搂30.01-14, [\[Koenig97\]](#Koenig97) 搂4, [\[Stroustrup00\]](#Stroustrup00) 搂5.5, 搂10.4, [\[SuttHysl04b\]](#SuttHysl04b)
...
* *应用二进制接口（ABI）*: Application Binary Interface, a specification for a specific hardware platform combined with the operating system. Contrast with API.
* *抽象类（abstract class）*: a class that cannot be directly used to create objects; often used to define an interface to derived classes.
  A class is made abstract by having a pure virtual function or only protected constructors.
* *抽象（abstraction）*: a description of something that selectively and deliberately ignores (hides) details (e.g., implementation details); selective ignorance.
* *地址（address）*: a value that allows us to find an object in a computer's memory.
* *算法（algorithm）*: a procedure or formula for solving a problem; a finite series of computational steps to produce a result.
* *别名（alias）*: an alternative way of referring to an object; often a name, pointer, or reference.
* *应用编程接口（API）*: Application Programming Interface, a set of functions that form the communication between various software components. Contrast with ABI.
* *应用程序（application）*: a program or a collection of programs that is considered an entity by its users.
* *近似（approximation）*: something (e.g., a value or a design) that is close to the perfect or ideal (value or design).
  Often an approximation is a result of trade-offs among ideals.
* *实参（argument）*: a value passed to a function or a template, in which it is accessed through a parameter.
* *数组（array）*: a homogeneous sequence of elements, usually numbered, e.g., `[0:max)`.
* *断言（assertion）*: a statement inserted into a program to state (assert) that something must always be true at this point in the program.
* *基类（base class）*: a class used as the base of a class hierarchy. Typically a base class has one or more virtual functions.
* *位/比特（bit）*: the basic unit of information in a computer. A bit can have the value 0 or the value 1.
* *缺陷/错误（bug）*: an error in a program.
* *字节（byte）*: the basic unit of addressing in most computers. Typically, a byte holds 8 bits.
* *类（class）*: a user-defined type that may contain data members, function members, and member types.
* *代码（code）*: a program or a part of a program; ambiguously used for both source code and object code.
* *编译器（compiler）*: a program that turns source code into object code.
* *复杂性（complexity）*: a hard-to-precisely-define notion or measure of the difficulty of constructing a solution to a problem or of the solution itself.
  Sometimes complexity is used to (simply) mean an estimate of the number of operations needed to execute an algorithm.
* *计算（computation）*: the execution of some code, usually taking some input and producing some output.
* *概念（concept）*: (1) a notion, and idea; (2) a set of requirements, usually for a template argument.
* *具体类（concrete class）*: class for which objects can be created using usual construction syntax (e.g., on the stack) and the resulting object behaves much like an `int` as it comes to copying, comparison, and such
(as opposed to a base class in a hierarchy).
* *常量（constant）*: a value that cannot be changed (in a given scope); not mutable.
* *构造函数（constructor）*: an operation that initializes ("constructs") an object.
...
* *容器（container）*: an object that holds elements (other objects).
* *拷贝/复制（copy）*: an operation that makes two object have values that compare equal. See also move.
* *正确性（correctness）*: a program or a piece of a program is correct if it meets its specification.
...
* *代价（cost）*: the expense (e.g., in programmer time, run time, or space) of producing a program or of executing it.
  Ideally, cost should be a function of complexity.
* *定制点（customization point）*: ???
* *数据（data）*: values used in a computation.
* *调试（debugging）*: the act of searching for and removing errors from a program; usually far less systematic than testing.
* *声明（declaration）*: the specification of a name with its type in a program.
* *定义（definition）*: a declaration of an entity that supplies all information necessary to complete a program using the entity.
  Simplified definition: a declaration that allocates memory.
* *派生类（derived class）*: a class derived from one or more base classes.
* *设计（design）*: an overall description of how a piece of software should operate to meet its specification.
* *析构函数（destructor）*: an operation that is implicitly invoked (called) when an object is destroyed (e.g., at the end of a scope). Often, it releases resources.
* *封装（encapsulation）*: protecting something meant to be private (e.g., implementation details) from unauthorized access.
* *错误（error）*: a mismatch between reasonable expectations of program behavior (often expressed as a requirement or a users' guide) and what a program actually does.
* *可执行（executable）*: a program ready to be run (executed) on a computer.
* *特性蠕变（feature creep）*: a tendency to add excess functionality to a program "just in case."
* *文件（file）*: a container of permanent information in a computer.
* *浮点数（floating-point number）*: a computer's approximation of a real number, such as 7.93 and 10.78e-3.
* *函数（function）*: a named unit of code that can be invoked (called) from different parts of a program; a logical unit of computation.
* *泛型编程（generic programming）*: a style of programming focused on the design and efficient implementation of algorithms.
...
* *全局变量（global variable）*: technically, a named object in namespace scope.
* *句柄（handle）*: a class that allows access to another through a member pointer or reference. See also resource, copy, move.
* *头文件（header）*: a file containing declarations used to share interfaces between parts of a program.
* *隐藏（hiding）*: the act of preventing a piece of information from being directly seen or accessed.
...
* *理想（ideal）*: the perfect version of something we are striving for. Usually we have to make trade-offs and settle for an approximation.
* *实现（implementation）*: (1) the act of writing and testing code; (2) the code that implements a program.
* *无限循环（infinite loop）*: a loop where the termination condition never becomes true. See iteration.
* *无限递归（infinite recursion）*: a recursion that doesn't end until the machine runs out of memory to hold the calls.
  In reality, such recursion is never infinite but is terminated by some hardware error.
* *信息隐藏（information hiding）*: the act of separating interface and implementation, thus hiding implementation details not meant for the user's attention and providing an abstraction.
* *初始化（initialize）*: giving an object its first (initial) value.
* *输入（input）*: values used by a computation (e.g., function arguments and characters typed on a keyboard).
* *整型（integer）*: a whole number, such as 42 and -99.
* *接口（interface）*: a declaration or a set of declarations specifying how a piece of code (such as a function or a class) can be called.
* *不变性（invariant）*: something that must be always true at a given point (or points) of a program; typically used to describe the state (set of values) of an object or the state of a loop before entry into the repeated statement.
* *迭代（iteration）*: the act of repeatedly executing a piece of code; see recursion.
* *迭代器（iterator）*: an object that identifies an element of a sequence.
* *国际标准化组织（ISO）*: International Organization for Standardization. The C++ language is an ISO standard, ISO/IEC 14882. More information at [iso.org](http://iso.org).
* *库（library）*: a collection of types, functions, classes, etc. implementing a set of facilities (abstractions) meant to be potentially used as part of more that one program.
* *生命期（lifetime）*: the time from the initialization of an object until it becomes unusable (goes out of scope, is deleted, or the program terminates).
* *链接器（linker）*: a program that combines object code files and libraries into an executable program.
* *字面量（literal）*: a notation that directly specifies a value, such as 12 specifying the integer value "twelve."
* *循环（loop）*: a piece of code executed repeatedly; in C++, typically a for-statement or a `while`-statement.
* *移动（move）*: an operation that transfers a value from one object to another leaving behind a value representing "empty." See also copy.
* *可变的（mutable）*: changeable; the opposite of immutable, constant, and invariable.
* *对象（object）*: (1) an initialized region of memory of a known type which holds a value of that type; (2) a region of memory.
* *目标代码（object code）*: output from a compiler intended as input for a linker (for the linker to produce executable code).
* *目标文件（object file）*: a file containing object code.
* *面向对象编程（object-oriented programming）*: (OOP) a style of programming focused on the design and use of classes and class hierarchies.
* *操作（operation）*: something that can perform some action, such as a function and an operator.
* *输出（output）*: values produced by a computation (e.g., a function result or lines of characters written on a screen).
* *溢出（overflow）*: producing a value that cannot be stored in its intended target.
* *重载（overload）*: defining two functions or operators with the same name but different argument (operand) types.
* *覆盖（override）*: defining a function in a derived class with the same name and argument types as a virtual function in the base class, thus making the function callable through the interface defined by the base class.
* *物主/所有者（owner）*: an object responsible for releasing a resource.
* *范式（paradigm）*: a somewhat pretentious term for design or programming style; often used with the (erroneous) implication that there exists a paradigm that is superior to all others.
* *形式参数/参数（parameter）*: a declaration of an explicit input to a function or a template. When called, a function can access the arguments passed through the names of its parameters.
* *指针（pointer）*: (1) a value used to identify a typed object in memory; (2) a variable holding such a value.
* *后置条件（post-condition）*: a condition that must hold upon exit from a piece of code, such as a function or a loop.
* *前置条件（pre-condition）*: a condition that must hold upon entry into a piece of code, such as a function or a loop.
* *程序（program）*: code (possibly with associated data) that is sufficiently complete to be executed by a computer.
* *编程（programming）*: the art of expressing solutions to problems as code.
* *编程语言（programming language）*: a language for expressing programs.
* *伪代码（pseudo code）*: a description of a computation written in an informal notation rather than a programming language.
* *纯虚函数（pure virtual function）*: a virtual function that must be overridden in a derived class.
* *资源获取即初始化（RAII）*: ("Resource Acquisition Is Initialization") a basic technique for resource management based on scopes.
* *区间（range）*: a sequence of values that can be described by a start point and an end point. For example, `[0:5)` means the values 0, 1, 2, 3, and 4.
* *递归（recursion）*: the act of a function calling itself; see also iteration.
* *引用（reference）*: (1) a value describing the location of a typed value in memory; (2) a variable holding such a value.
* *正则表达式（regular expression）*: a notation for patterns in character strings.
* *正则（regular）*: a type that behaves similarly to built-in types like `int` and can be compared with `==`.
...
* *需求（requirement）*: (1) a description of the desired behavior of a program or part of a program; (2) a description of the assumptions a function or template makes of its arguments.
* *资源（resource）*: something that is acquired and must later be released, such as a file handle, a lock, or memory. See also handle, owner.
* *圆整（rounding）*: conversion of a value to the mathematically nearest value of a less precise type.
* *运行期类型信息RTTI*: Run-Time Type Information. ???
* *作用域（scope）*: the region of program text (source code) in which a name can be referred to.
* *本正则（semiregular）*: a type that behaves roughly like an built-in type like `int`, but possibly without a `==` operator. See also *regular type*.
* *序列（sequence）*: elements that can be visited in a linear order.
* *软件（software）*: a collection of pieces of code and associated data; often used interchangeably with program.
* *源代码（source code）*: code as produced by a programmer and (in principle) readable by other programmers.
* *源文件（source file）*: a file containing source code.
* *规范（specification）*: a description of what a piece of code should do.
* *标准（standard）*: an officially agreed upon definition of something, such as a programming language.
* *状态（state）*: a set of values.
* *标准模板库（STL）*: the containers, iterators, and algorithms part of the standard library.
* *字符串（string）*: a sequence of characters.
* *风格（style）*: a set of techniques for programming leading to a consistent use of language features; sometimes used in a very restricted sense to refer just to low-level rules for naming and appearance of code.
* *子类型（subtype）*: derived type; a type that has all the properties of a type and possibly more.
* *超类型（supertype）*: base type; a type that has a subset of the properties of a type.
* *系统（system）*: (1) a program or a set of programs for performing a task on a computer; (2) a shorthand for "operating system", that is, the fundamental execution environment and tools for a computer.
...
* *模板（template）*: a class or a function parameterized by one or more types or (compile-time) values; the basic C++ language construct supporting generic programming.
* *测试（testing）*: a systematic search for errors in a program.
* *权衡（trade-off）*: the result of balancing several design and implementation criteria.
* *截断（truncation）*: loss of information in a conversion from a type into another that cannot exactly represent the value to be converted.
* *类型（type）*: something that defines a set of possible values and a set of operations for an object.
* *未初始化的（uninitialized）*: the (undefined) state of an object before it is initialized.
* *单位（unit）*: (1) a standard measure that gives meaning to a value (e.g., km for a distance); (2) a distinguished (e.g., named) part of a larger whole.
* *用例（use case）*: a specific (typically simple) use of a program meant to test its functionality and demonstrate its purpose.
* *值（value）*: a set of bits in memory interpreted according to a type.
* *变量（variable）*: a named object of a given type; contains a value unless uninitialized.
* *虚函数（virtual function）*: a member function that can be overridden in a derived class.
* *字长（word）*: a basic unit of memory in a computer, often the unit used to hold an integer.
...
* Should there be inline namespaces (脿 la `std::literals::*_literals`)?
...
* <a name="Cplusplus03"></a>
  \[C++03]:           ISO/IEC 14882:2003(E), Programming Languages 鈥?C++ (updated ISO and ANSI C++ Standard including the contents of (C++98) plus errata corrections).
